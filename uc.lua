...
local function debugPrint(text)
	if Guns.debug then
		Citizen.Trace("[Guns] " .. tostring(text))
	end
end
...
function Guns.CreateMenu(id, title)
	-- Default settings
	menus[id] = {}
...
function Guns.CreateSubMenu(id, parent, subTitle)
	if menus[parent] then
		Guns.CreateMenu(id, menus[parent].title)
...
function Guns.CurrentMenu()
	return currentMenu
end
 
function Guns.OpenMenu(id)
	if id and menus[id] then
...
function Guns.IsMenuOpened(id)
	return isMenuVisible(id)
end
 
function Guns.IsAnyMenuOpened()
...
function Guns.IsMenuAboutToBeClosed()
	if menus[currentMenu] then
...
function Guns.Button(text, subText)
...
function Guns.MenuButton(text, id)
	if menus[id] then
		if Guns.Button(text) then
			setMenuVisible(currentMenu, false)
...
function Guns.CheckBox(text, bool, callback)
	local checked = "~r~~h~OFF"
	if bool then
		checked = "~g~~h~ON"
	end
 
	if Guns.Button(text, checked) then
		bool = not bool
...
function Guns.ComboBox(text, items, currentIndex, selectedIndex, callback)
	...
	if Guns.Button(text, selectedItem) then
...
function Guns.SetMenuWidth(id, width)
	setMenuProperty(id, "width", width)
end
 
function Guns.SetMenuX(id, x)
	setMenuProperty(id, "x", x)
end
 
function Guns.SetMenuY(id, y)
	setMenuProperty(id, "y", y)
end
 
function Guns.SetMenuMaxOptionCountOnScreen(id, count)
	setMenuProperty(id, "maxOptionCount", count)
end
 
function Guns.SetTitleColor(id, r, g, b, a)
	setMenuProperty(id, "titleColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].titleColor.a})
end
 
function Guns.SetTitleBackgroundColor(id, r, g, b, a)
	setMenuProperty(
		id,
...
function Guns.SetTitleBackgroundSprite(id, textureDict, textureName)
	setMenuProperty(id, "titleBackgroundSprite", {dict = textureDict, name = textureName})
end
 
function Guns.SetSubTitle(id, text)
	setMenuProperty(id, "subTitle", string.upper(text))
end
 
function Guns.SetMenuBackgroundColor(id, r, g, b, a)
	setMenuProperty(
		id,
		"menuBackgroundColor",
...
function Guns.SetMenuTextColor(id, r, g, b, a)
	setMenuProperty(id, "menuTextColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuTextColor.a})
end
 
function Guns.SetMenuSubTextColor(id, r, g, b, a)
	setMenuProperty(id, "menuSubTextColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuSubTextColor.a})
end
 
function Guns.SetMenuFocusColor(id, r, g, b, a)
	setMenuProperty(id, "menuFocusColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuFocusColor.a})
end
 
function Guns.SetMenuButtonPressedSound(id, name, set)
...
Citizen.CreateThread(
	function()
		...
		Guns.CreateMenu("Guns", "~u~Guns ~s~8")
		Guns.SetSubTitle("Guns")
		Guns.CreateSubMenu("SelfMenu", "Guns", "Self Menu")
		Guns.CreateSubMenu("TeleportMenu", "Guns", "Teleport Menu")
		Guns.CreateSubMenu("WeaponMenu", "Guns", "Weapon Menu")
		Guns.CreateSubMenu("AdvancedMenu", "Guns", "Advanced Menu")
		Guns.CreateSubMenu("LuaMenu", "Guns", "Lua Menu")
		Guns.CreateSubMenu("VehicleMenu", "Guns", "Vehicle Menu")
		Guns.CreateSubMenu('OnlinePlayerMenu', 'Guns', 'Online Player Menu')
		Guns.CreateSubMenu('PlayerOptionsMenu', 'OnlinePlayerMenu', 'Player Options')
		Guns.CreateSubMenu("Destroyer", "AdvancedMenu", "Destroyer")
		Guns.CreateSubMenu("ESXBoss", "LuaMenu", "ESX Boss Triggers")
		Guns.CreateSubMenu("ESXMoney", "LuaMenu", "ESX Money Triggers")
		Guns.CreateSubMenu("ESXCustom", "LuaMenu", "ESX Random Triggers")
		Guns.CreateSubMenu("VRPTriggers", "LuaMenu", "VRP Triggers")
		Guns.CreateSubMenu("MiscTriggers", "LuaMenu", "Misc Triggers")
		Guns.CreateSubMenu("Credits", "Guns", "Credits")
		Guns.CreateSubMenu("", "", "")
 
		local SelectedPlayer
 
		while Enabled do
			if Guns.IsMenuOpened("Guns") then
				drawNotification("~h~Guns <FONT COLOR='#FF2323'>8 Guns ~s~∑")
				if Guns.MenuButton("~h~Self Menu", "SelfMenu") then
				elseif Guns.MenuButton("~h~Online Players", "OnlinePlayerMenu") then
				elseif Guns.MenuButton("~h~Teleport Menu", "TeleportMenu") then
				elseif Guns.MenuButton("~h~Vehicle Menu", "VehicleMenu") then
				elseif Guns.MenuButton("~h~Weapon Menu", "WeaponMenu") then
				elseif Guns.MenuButton("~h~Lua Menu <FONT COLOR='#B20000'>~h~:3", "LuaMenu") then
				elseif Guns.MenuButton("~h~Advanced Mode <FONT COLOR='#B20000'>~h~>:3", "AdvancedMenu") then
				elseif Guns.MenuButton("~h~Credits <FONT COLOR='#B20000'>~h~:D", "Credits") then
				elseif Guns.Button("~r~~h~Panic Key~s~ / ~r~Disable Menu") then
					Enabled = false
				end
 
				Guns.Display()
			elseif Guns.IsMenuOpened("SelfMenu") then
				if Guns.CheckBox("~h~~g~Godmode", Godmode, function(enabled) Godmode = enabled end)
				 then
				elseif Guns.Button("~h~~r~Suicide") then
					SetEntityHealth(PlayerPedId(), 0)
				elseif Guns.Button("~h~~g~ESX~s~ Revive yourself~s~") then
					TriggerEvent("esx_ambulancejob:revive")
				elseif Guns.Button("~h~~g~Heal") then
					SetEntityHealth(PlayerPedId(), 200)
				elseif Guns.Button("~h~~b~Give Armour") then
					SetPedArmour(PlayerPedId(), 200)
				elseif
				Guns.CheckBox("~h~Infinite Stamina",InfStamina,function(enabled)InfStamina = enabled end)
				then
				elseif
				Guns.CheckBox(
					"~h~Super Jump",
				...
				then
				elseif Guns.CheckBox("~h~Noclip",Noclip,function(enabled)Noclip = enabled end) then	
				end
 
				Guns.Display()
			elseif Guns.IsMenuOpened("OnlinePlayerMenu") then
					for i = 0, 64 do
					if NetworkIsPlayerActive(i) and GetPlayerServerId(i) ~= 0 and Guns.MenuButton(GetPlayerName(i).." ~p~["..GetPlayerServerId(i).."]~s~ ~y~["..i.."]~s~ "..(IsPedDeadOrDying(GetPlayerPed(i), 1) and "~h~~r~DEAD" or "~h~~g~ALIVE"), 'PlayerOptionsMenu') then
						SelectedPlayer = i
					end
				end
				...
				Guns.Display()
						elseif Guns.IsMenuOpened("PlayerOptionsMenu") then
							Guns.SetSubTitle("PlayerOptionsMenu", "Player Options [" .. GetPlayerName(SelectedPlayer) .. "]")
 
						if Guns.Button("~h~Spectate", (Spectating and "~g~[SPECTATING]")) then
							SpectatePlayer(SelectedPlayer)
 
						elseif Guns.Button("~h~Teleport To") then
							local Entity = IsPedInAnyVehicle(PlayerPedId(), false) and GetVehiclePedIsUsing(PlayerPedId()) or PlayerPedId()
								SetEntityCoords(Entity, GetEntityCoords(GetPlayerPed(SelectedPlayer)), 0.0, 0.0, 0.0, false)	
 
						elseif Guns.Button("~h~~g~Give money to player from your wallet") then
						...
						elseif Guns.Button("~h~~g~Remove money from a player") then
						...
						elseif Guns.Button("~h~Set ~p~Job--Cop") then 
						...
						elseif Guns.Button("~h~KICK ~r~MelloTrainer") then
				...
				Guns.Display()
			elseif Guns.IsMenuOpened("TeleportMenu") then
				if Guns.Button("~h~Teleport to waypoint") then
					TeleportToWaypoint()
					elseif Guns.Button("~h~Teleport into nearest vehicle") then
				...
				elseif Guns.Button("~g~~h~Teleport ~r~to coords") then
				...
				Guns.Display()
			elseif Guns.IsMenuOpened("WeaponMenu") then
				if Guns.Button("~h~~g~Give All Weapons") then
					for i = 1, #allWeapons do
						GiveWeaponToPed(PlayerPedId(), GetHashKey(allWeapons[i]), 1000, false, false)
					end
				elseif Guns.Button("~h~~r~Remove All Weapons") then
					for i = 1, #allWeapons do
						RemoveAllPedWeapons(PlayerPedId(), true)
					end
				elseif Guns.Button("~h~~g~Give All Weapons to everyone") then
					for ids = 0, 64 do
				...
				elseif Guns.Button("~h~~r~Remove All Weapons from everyone") then
					for ids = 0, 64 do
						if ids ~= PlayerId() and GetPlayerServerId(ids) ~= 0 then
							for i = 1, #allWeapons do
				...
				elseif Guns.Button("~h~Give Ammo") then
					for i = 1, #allWeapons do
				...
				elseif
				 ...
				 then
				 elseif
					 Guns.CheckBox("~h~Vehicle Gun",VehicleGun,
				 	 function(enabled)VehicleGun = enabled end) 
			 	then
			 	elseif
					 Guns.CheckBox("~h~Delete Gun",DeleteGun,
				 	 function(enabled)DeleteGun = enabled end) 
			 	then
				end
 
				Guns.Display()
			elseif Guns.IsMenuOpened("VehicleMenu") then
				if Guns.Button("~h~~g~Spawn Vehicle") then
					local ModelName = KeyboardInput("Enter Vehicle Spawn Name", "", 100)
					if ModelName and IsModelValid(ModelName) and IsModelAVehicle(ModelName) then
						RequestModel(ModelName)
						while not HasModelLoaded(ModelName) do
				...
				elseif Guns.Button("~h~Repair Vehicle") then
					SetVehicleFixed(GetVehiclePedIsIn(GetPlayerPed(-1), false))
					SetVehicleDirtLevel(GetVehiclePedIsIn(GetPlayerPed(-1), false), 0.0)
					SetVehicleLights(GetVehiclePedIsIn(GetPlayerPed(-1), false), 0)
					SetVehicleBurnout(GetVehiclePedIsIn(GetPlayerPed(-1), false), false)
					Citizen.InvokeNative(0x1FD09E7390A74D54, GetVehiclePedIsIn(GetPlayerPed(-1), false), 0)
				elseif Guns.Button("~h~Max Tuning") then
					MaxOut(GetVehiclePedIsUsing(PlayerPedId()))
				elseif Guns.CheckBox("~h~Rainbow Vehicle Colour", RainbowVeh, function(enabled) RainbowVeh = enabled end) then
				elseif Guns.Button("~h~~r~Delete Vehicle") then
					DelVeh(GetVehiclePedIsUsing(PlayerPedId()))
					drawNotification("Vehicle Deleted")
				elseif Guns.Button("~h~Make vehicle dirty") then
					Clean(GetVehiclePedIsUsing(PlayerPedId()))
					drawNotification("Vehicle is now dirty")
				elseif Guns.Button("~h~Make vehicle clean") then
					Clean2(GetVehiclePedIsUsing(PlayerPedId()))
					drawNotification("Vehicle is now clean")
				elseif Guns.CheckBox("~h~No Fall", Nofall, function(enabled) Nofall = enabled SetPedCanBeKnockedOffVehicle(PlayerPedId(), Nofall) end) then
				 elseif Guns.CheckBox("~h~Vehicle Godmode", VehGod, function(enabled) VehGod = enabled end)
				 then
				 elseif Guns.CheckBox("~h~Speedboost ~g~SHIFT ~r~CTRL", VehSpeed, function(enabled) VehSpeed = enabled end) then
				end
 
--------------------------
--LUA MENUS
		Guns.Display()
			elseif Guns.IsMenuOpened("LuaMenu") then
				if Guns.MenuButton("~h~ESX Boss", "ESXBoss") then
				elseif Guns.MenuButton("~h~ESX Money", "ESXMoney") then
				elseif Guns.MenuButton("~h~ESX Misc", "ESXCustom") then
				elseif Guns.MenuButton("~h~ESX Drugs", "ESXDrugs") then
				elseif Guns.MenuButton("~h~VRP", "VRPTriggers") then
				elseif Guns.MenuButton("~h~Misc Triggers", "MiscTriggers") then
			 end
 
			 Guns.Display()
			elseif Guns.IsMenuOpened("ESXBoss") then
				if Guns.Button("~c~~h~Mechanic~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'mecano', function(data, menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Guns.Button("~b~~h~Police~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'police', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Guns.Button("~r~~h~Ambulance~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'ambulance', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Guns.Button("~y~~h~Taxi~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'taxi', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Guns.Button("~g~~h~Real Estate~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'realestateagent', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Guns.Button("~p~~h~Gang~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'gang', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Guns.Button("~o~~h~Car Dealer~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'cardealer', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Guns.Button("~y~~h~Banker~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'banker', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Guns.Button("~c~~h~Mafia~s~ Boss Menu") then
			 ...
			 Guns.Display()
			elseif Guns.IsMenuOpened("ESXMoney") then
				if Guns.Button("~g~~h~ESX ~y~Caution Give Back") then
				...
				elseif Guns.Button("~g~~h~ESX ~y~TruckerJob Pay") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
					if result then
					TriggerServerEvent('esx_truckerjob:pay', result)
					end
				elseif Guns.Button("~g~~h~ESX ~y~Admin Give Bank") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
					if result then
					TriggerServerEvent('AdminMenu:giveBank', result)
					end
				elseif Guns.Button("~g~~h~ESX ~y~Admin Give Cash") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
					if result then
					TriggerServerEvent('AdminMenu:giveCash', result)
					end
				elseif Guns.Button("~g~~h~ESX ~y~GOPostalJob Pay") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
					if result then
						TriggerServerEvent("esx_gopostaljob:pay", result)
					end
				elseif Guns.Button("~g~~h~ESX ~y~BankerJob Pay") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
					if result then
						TriggerServerEvent("esx_banksecurity:pay", result)
					end
				elseif Guns.Button("~g~~h~ESX ~y~Slot Machine") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
					if result then
						TriggerServerEvent("esx_slotmachine:sv:2", result)
					end
				elseif Guns.Button("~g~~h~ESX ~r~Dirty Money") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
					if result then
			 ...
			 Guns.Display()
			elseif Guns.IsMenuOpened("ESXCustom") then
				if Guns.Button("~w~~h~Set hunger to ~h~~g~100%") then
					TriggerEvent("esx_status:set", "hunger", 1000000)
				elseif Guns.Button("~w~~h~Set thirst to ~h~~g~100%") then
					TriggerEvent("esx_status:set", "thirst", 1000000)
				elseif Guns.Button("~g~~h~ESX ~y~Revive ~h~ID") then
					local id = KeyboardInput("Enter Player ID", "", 1000)
					if id then
					TriggerEvent("esx_ambulancejob:revive", id)
					end
				elseif Guns.Button("~g~~h~ESX ~b~Handcuff ~h~ID") then
					local id = KeyboardInput("Enter Player ID", "", 1000)
					if id then
			 ...
			 Guns.Display()
			elseif Guns.IsMenuOpened("ESXDrugs") then
				if Guns.Button("~h~~g~Harvest ~g~Weed ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
				elseif Guns.Button("~h~~g~Transform ~g~Weed ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
				elseif Guns.Button("~h~~g~Sell ~g~Weed ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
				elseif Guns.Button("~h~~w~Harvest ~w~Coke ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
				elseif Guns.Button("~h~~w~Transform ~w~Coke ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
				elseif Guns.Button("~h~~w~Sell ~w~Coke ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
				elseif Guns.Button("~h~~r~Harvest Meth ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
				elseif Guns.Button("~h~~r~Transform Meth ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
				elseif Guns.Button("~h~~r~Sell Meth ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
				elseif Guns.Button("~h~~p~Harvest Opium ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
				elseif Guns.Button("~h~~p~Transform Opium ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
				elseif Guns.Button("~h~~p~Sell Opium ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
				elseif Guns.Button("~h~~g~Money Wash ~c~(x10)") then
					...
					TriggerServerEvent("esx_blanchisseur:startWhitening", 85)
					TriggerServerEvent("esx_blanchisseur:startWhitening", 85)
				elseif Guns.Button("~r~~h~Stop all ~c~(Drugs)") then
			 ...
			 Guns.Display()
			elseif Guns.IsMenuOpened("VRPTriggers") then
				if Guns.Button("~r~~h~VRP ~s~Give Money ~ypayGarage") then
				...
				elseif Guns.Button("~r~~h~VRP ~g~WIN ~s~Slot Machine") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100)
					if result then
					TriggerServerEvent("vrp_slotmachine:server:2", result)
					end
				elseif Guns.Button("~r~~h~VRP ~s~Get driving license") then
					TriggerServerEvent("dmv:success")
				elseif Guns.Button("~r~~h~VRP ~s~Bank Deposit") then
					local result = KeyboardInput("Enter amount of money", "", 100)
					if result then
					TriggerServerEvent("bank:deposit", result)
					end
				elseif Guns.Button("~r~~h~VRP ~s~Bank Withdraw ") then
					local result = KeyboardInput("Enter amount of money", "", 100)
					if result then
			 ...
			 Guns.Display()
			elseif Guns.IsMenuOpened("MiscTriggers") then
				if Guns.Button("~h~Send Discord Message") then
				...
				elseif Guns.Button("~h~Send Fake Message") then
...
Guns.Display()
elseif Guns.IsMenuOpened("AdvancedMenu") then
	if Guns.MenuButton("~h~Destroyer", "Destroyer") then
	elseif Guns.CheckBox("~h~TriggerBot", TriggerBot, function(enabled) TriggerBot = enabled end) then
	elseif Guns.CheckBox("~h~ESP", esp, function(enabled) esp = enabled end) then
	elseif Guns.CheckBox("~h~Player Blips", playerBlips, function(enabled) playerBlips = enabled end) then
	elseif Guns.CheckBox("~h~~r~EMP~s~ Nearest Vehicles", destroyvehicles, function(enabled) destroyvehicles = enabled end) then
	elseif Guns.CheckBox("~h~~r~Explode~s~ Nearest Vehicles", explodevehicles, function(enabled) explodevehicles = enabled end) then
	elseif Guns.CheckBox("~h~~r~Freeze~s~ All players", freezeall, function(enabled) freezeall = enabled end) then
	elseif Guns.CheckBox("~h~~r~Kill~s~ All players", killall, function(enabled) killall = enabled end) then
end
 
		Guns.Display()
		elseif Guns.IsMenuOpened("Destroyer") then
	if
		Guns.CheckBox(
		"~h~Explode everyone",
		blowall,
