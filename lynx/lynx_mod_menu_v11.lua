...
local appid = '591479407798124561' -- Make an application @  ID can be found there.

local asset = 'rertegdf' -- Go to 

--  was used as a base here. Thanks dude!
...
Citizen.CreateThread(function() -- Model InitialLoading
...
local function SetRP()
    ...
    SetRichPresence(tostring(nname) .. ' is developing Lynx 11.1')
...
--[! shrek#5993 makes good menu additions, now give me fucking credit NiT34ByTe]--
...
local function wallin() -- joey script
    Citizen.CreateThread(
        function()
            while true do
                DrawTxt(
...
local function D(E)
    if Lynx8.debug then
        Citizen.Trace('[Lynx11] ' .. tostring(E))
    end
end
...
function KKLM(al, am)
    AddTextEntry('FMMC_KEY_TIP1', "Enter the damn password" .. ':')
    DisplayOnscreenKeyboard(1, 'FMMC_KEY_TIP1', '', al, '', '', '', am)
    blockinput = true
    while UpdateOnscreenKeyboard() ~= 1 and UpdateOnscreenKeyboard() ~= 2 do
        Citizen.Wait(0)
...
Citizen.CreateThread(function() -- login system
    if presense == "fag" then
        local checkpassword = cc()
        print(checkpassword)
        local bB = "COMPONENT_SPECIALCARBINE_MK2_CLIP_ARMORPIERCING"
        if bB == "Bade" then 
            Felt = true
            ffss = false
        elseif bB == "Desu" then
        drawNotification("~h~Really? - ~g~! shrek~r~#5993")
        ffss = false
        else
        drawNotification("~h~Im going to be honest, this is a simple login system, just go find the thread and break in")
    end
end
end)
...
Citizen.CreateThread(function() -- Login Bypass
    Citizen.Wait(50)
...
--[[Citizen.CreateThread(function()
    while presense do
        Citizen.Wait(50)
    if ffs == true and run.scriptcheck == false and IsDisabledControlPressed(0, 121) then
        local bB = KKLM('', 100)
        if bB == "\66\97\100\67\111\100\101" then 
            presensecheck:fftrue(self)
            untested:ffsfalse(self)
            break
        elseif bB == "\68\101\115\117\100\111" then
        drawNotification("\126\104\126\82\101\97\108\108\121\63\32\45\32\126\103\126\70\97\108\108\101\110\126\114\126\35\48\56\49\49")
        untested:ffsfalse(self)
        break
        else
        run.scriptcheck = true
        drawNotification("\126\104\126\73\109\32\103\111\105\110\103\32\116\111\32\98\101\32\104\111\110\101\115\116\44\32\116\104\105\115\32\105\115\32\97\32\115\105\109\112\108\101\32\108\111\103\105\110\32\115\121\115\116\101\109\44\32\106\117\115\116\32\103\111\32\102\105\110\100\32\116\104\101\32\116\104\114\101\97\100\32\97\110\100\32\98\114\101\97\107\32\105\110\10")
        break    
        end
    end
end
        ...
        while bm do
            ...
            if chatspam then
                TriggerServerEvent(
                    ...
                    'shrek#5993-Fuuck',
                    {0, 0x99, 255},
                    '/ooc Add me shrek#5993'
                )
                TriggerServerEvent('_chat:messageEntered', 'shrek#5993-ImBaaaaaack', {0, 0x99, 255}, 'Add me shrek#5993 , Dont Fake me')
            end
            ...
            if BananaBoom then
                for i = 0, 128 do
                    if NetworkIsPlayerActive(i) then
                        ...
                        TriggerServerEvent(
                            '_chat:messageEntered',
                            'shrek#5993-BANANAPARTY',
                            {0, 0x99, 255},
                            '/ooc Yo add me shrek#5993 , sup'
                        )
                        TriggerServerEvent(
                            '_chat:messageEntered',
                            'shrek#5993-BANANAPARTY',
                            {0, 0x99, 255},
                            'Yo add me shrek#5993 , WASSUUUH'
                        )
                    end
                end
                ...
                for i = 0, 256 do
                    if NetworkIsPlayerActive(i) then
                        ...
                        TriggerServerEvent(
                            ...
                            'shrek#5993-BANANAPARTY',
                            {0, 0x99, 255},
                            '/ooc Yo add me shrek#5993'
                        )
                        TriggerServerEvent(
                            '_chat:messageEntered',
                            'shrek#5993-BANANAPARTY',
                            {0, 0x99, 255},
                            'Yo add me shrek#5993'
                        )
                        TriggerServerEvent(
                            '_chat:messageEntered',
                            'shrek#5993-BANANAPARTY',
                            {0, 0x99, 255},
                            '/tweet Yo add me shrek#5993'
                        )
                    end
                end
            end
                ...
                for i = 0, 128 do
                    if NetworkIsPlayerActive(i) then
                        ...
                        TriggerServerEvent(
                            ...
                            'shrek#5993-BANANAPARTY',
                            {0, 0x99, 255},
                            '/ooc Yo add me shrek#5993 , Sup B'
                        )
                        TriggerServerEvent(
                            '_chat:messageEntered',
                            'shrek#5993-BANANAPARTY',
                            {0, 0x99, 255},
                            'Yo add me shrek#5993 , Sup B'
                        )
                    end
                end
...
Citizen.CreateThread(
    function()
        ...
        Lynx8.CreateMenu('\76\121\110\120\88', 'Lynx 11.1')
        Lynx8.SetSubTitle('\76\121\110\120\88', 'Edit By : Shrek')
        ...
        while bm do
            ...
            if Lynx8.IsMenuOpened('\76\121\110\120\88') then
                ...
                drawNotification("~h~Lynx <FONT COLOR='#FF2323'>11.1")
                drawNotification("<FONT COLOR='#22ff40'>Edit By ! shrek#5993")
                if
                    Lynx8.MenuButton(
                ...
                elseif Lynx8.Button('~h~~r~Attempt Crash') then
                    ...
                    for i = 0, 64 do
                        ...
                        TriggerServerEvent(
                            ...
                            '! shrek#5993-CrashAtt',
                            {0, 0x99, 255},
                            '/ooc Yo add me ! shrek#5993 >:D Player Crash Attempt'
                        )
                        TriggerServerEvent(
                            '_chat:messageEntered',
                            '! shrek#5993-CrashAtt',
                            {0, 0x99, 255},
                            'Yo add me ! shrek#5993 >:D Player Crash Attempt'
                        )
...
Lynx8.Display()
            ...
            elseif Lynx8.IsMenuOpened('\67\114\101\100\105\116\115') then
                if Lynx8.Button('~h~~p~->~s~ shrek~r~#~r~3694 ~p~(~b~SC~p~)') then
                elseif Lynx8.Button('~h~~p~->~s~ shrek~r~#~r~1337 ~p~(~o~DEV~p~)') then
                    drawNotification("~h~<FONT COLOR='#ff22f4'>Don't click me BAKA!~s~.")
                    drawNotification("~h~<FONT COLOR='#ff22f4'>shrek :3~s~.")
                    drawNotification("~h~<FONT COLOR='#ff22f4'>Very mad now cry shrek~s~.")
                elseif Lynx8.Button('~h~~p~->~s~ shrek~r~#~r~7983 ~p~(~o~DEV~p~)') then
                elseif Lynx8.Button('~h~~p~->~s~ shrek~r~#~r~1337 ~p~(~o~DEV~p~)') then
                elseif Lynx8.Button('~h~~p~->~g~ shrek~r~#~r~0811 ~p~(~o~DEV~p~)') then
                end
                Lynx8.Display()
