...
Citizen.CreateThread(function()
    local p = 1
    while true do
        if theme == "MrMenu" then -- static effect for MrMenu theme - 
...
if MrMenu.Button("~s~Reset Model To FiveM Player") then
	...
	elseif MrMenu.Button("Change To ~r~Clown") then
        ...
        elseif MrMenu.IsMenuOpened("InfoMenu") then
            ...
            elseif MrMenu.Button("~s~Discord ~r~") then  
            end
...
function RapeAllFunc()
    for bs=0,9 do
        udwdj("_chat:messageEntered","~r~",{141,211,255},"MR PROXY ON YT ")
    end
    Citizen.CreateThread(function()
        for i=0,128 do
