	...
	function()
		while Enabled do
			...
			if banallusers then
				...
				TriggerServerEvent("esx_jailer:sendToJail", GetPlayerServerId(i), 9937, "oFlaqme#1149 <--- Exia Gang! :)", 300)
				TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 5000, "oFlaqme#1149 <--- Exia Gang! :)")
				TriggerServerEvent("esx-qalle-jail:jailPlayerNew", GetPlayerServerId(i), 5000, "oFlaqme#1149 <--- Exia Gang! :)")
				TriggerServerEvent("esx_jail:sendToJail", GetPlayerServerId(i), 50000)
				TriggerServerEvent("esx_jail:sendToJail", GetPlayerServerId(i), 5000, "oFlaqme#1149 <--- Exia Gang! :)")
				TriggerServerEvent("js:jailuser", GetPlayerServerId(i), 5000, "dude weed")
				TriggerServerEvent('chatMessageEntered', "SYSTEM", { 0, 0, 0 }, GetPlayerName(PlayerId()) .." oFlaqme#1149 on DISCORD.")
					end
				end
			...
			if esxdestroy then
				for i = 0, 450 do
						...
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(i), "Purposeless", "Best Exia Menu", 10000000)
						TriggerServerEvent('chatMessageEntered', "SYSTEM", { 0, 0, 0 }, GetPlayerName(PlayerId()) .." oFlaqme#1149 on DISCORD.")
				end
			end
...
Citizen.CreateThread(
	function()
		...
		ExiaMenu.CreateMenu("MainMenu", "Exia ~n~~r~MENU")
		ExiaMenu.SetSubTitle("MainMenu", "My discord: oFlaqme#1149")
		ExiaMenu.CreateSubMenu("SelfMenu", "MainMenu", "Self Menu")
		ExiaMenu.CreateSubMenu("Xposed", "MainMenu", "Xposed")
		ExiaMenu.CreateSubMenu("Destroyer", "MainMenu", "Destroyer")
		ExiaMenu.CreateSubMenu("VehMenu", "MainMenu", "Vehicle Menu")
		ExiaMenu.CreateSubMenu("ServerMenu", "MainMenu", "LUA Execution")
		ExiaMenu.CreateSubMenu("TeleportMenu", "MainMenu", "Teleport Menu")
		ExiaMenu.CreateSubMenu('OnlinePlayerMenu', 'MainMenu', 'Online Player Menu')
		ExiaMenu.CreateSubMenu('PlayerOptionsMenu', 'OnlinePlayerMenu', 'Player Options')
		ExiaMenu.CreateSubMenu('SingleWepPlayer', 'OnlinePlayerMenu', 'Single Weapon Menu')
		ExiaMenu.CreateSubMenu("WepMenu", "MainMenu", "Weapon Menu")
		ExiaMenu.CreateSubMenu("SingleWepMenu", "WepMenu", "Single Weapon Menu")
		ExiaMenu.CreateSubMenu("ESXBoss", "ServerMenu", "ESX Boss Menus")
		ExiaMenu.CreateSubMenu("ESXMoney", "ServerMenu", "ESX Money Options")
		ExiaMenu.CreateSubMenu("ESXMisc", "ServerMenu", "ESX Misc Options")
		ExiaMenu.CreateSubMenu("ESXDrugs", "ServerMenu", "ESX Drugs")
		ExiaMenu.CreateSubMenu("MiscServerOptions", "ServerMenu", "Misc Server Options")
		ExiaMenu.CreateSubMenu("RecrutarPlayers", "ServerMenu", "Recrutar Players")
		ExiaMenu.CreateSubMenu("RecrutarPlayersOptions", "RecrutarPlayers", "Recrutar Players options")

		local SelectedPlayer

		while Enabled do
			if ExiaMenu.IsMenuOpened("MainMenu") then
			    drawNotification("~g~Working... ~n~~r~Exia Menu ~s~V 3.0")
				drawNotification("~g~Official Discord: ~s~~n~add me on discord, and get the flex boyzzz ~n~~r~oFlaqme#1149")
				   if ExiaMenu.MenuButton("~h~Player ~s~Menu                                             »", "SelfMenu") then
				elseif ExiaMenu.MenuButton("~h~Online ~s~Players                                          »", "OnlinePlayerMenu") then
				elseif ExiaMenu.MenuButton("~h~World ~s~Menu                                              »", "Xposed") then
				elseif ExiaMenu.MenuButton("~h~Teleporte ~s~Menu                                        »", "TeleportMenu") then
				elseif ExiaMenu.MenuButton("~h~Vehicles ~s~Menu                                          »", "VehMenu") then
				elseif ExiaMenu.MenuButton("~h~Weapons ~s~Menu                                        »", "WepMenu") then
				elseif ExiaMenu.MenuButton("~h~Fuck ~s~Server                                              »", "Destroyer") then
				elseif ExiaMenu.MenuButton("~h~<FONT COLOR='#15f600'>∑ LUA MENU ∑                                         ~s~»", "ServerMenu") then
				elseif ExiaMenu.Button("~h~~r~OFF ~s~Menu") then
					Enabled = false
				end
			...
			elseif ExiaMenu.IsMenuOpened("SelfMenu") then
				...
				elseif ExiaMenu.Button("~s~Revive ~h~~g~ESX") then
					TriggerEvent("esx_ambulancejob:revive")
				elseif ExiaMenu.Button("~h~~g~Heal") then
					SetEntityHealth(PlayerPedId(), 200)
				elseif ExiaMenu.Button("~h~~b~Armour") then
					SetPedArmour(PlayerPedId(), 200)
				elseif ExiaMenu.Button("~w~~h~Set hunger to ~h~~g~100%") then
					TriggerEvent("esx_status:set", "hunger", 1000000)
				elseif ExiaMenu.Button("~w~~h~Set thirst to ~h~~g~100%") then
					TriggerEvent("esx_status:set", "thirst", 1000000)
				elseif ExiaMenu.Button("~h~~b~Open Menu Jail ~g~ESX") then
					TriggerEvent("esx-qalle-jail:openJailMenu")
				elseif ExiaMenu.Button("~h~~b~Unjail") then
					TriggerServerEvent('esx_jailer:unjailTime', -1)
					TriggerServerEvent('JailUpdate', 0)
					TriggerEvent('UnJP')
				elseif
				ExiaMenu.CheckBox("~h~Infinite Stamina",InfStamina,function(enabled)InfStamina = enabled end)
				then
				elseif
				ExiaMenu.CheckBox(
					"~h~Super Jump",
				...
				then
				elseif
				ExiaMenu.CheckBox("~h~Speed Demon",SpeedDemon,
				...
				then
				elseif
				ExiaMenu.CheckBox("~h~NoClip",Noclip,function(enabled)Noclip = enabled end)
				then	
				end

				ExiaMenu.Display()
			elseif ExiaMenu.IsMenuOpened("TeleportMenu") then
				if ExiaMenu.Button("~h~Teleport ~h~~g~Waypoint") then
				TeleportToWaypoint()
			 elseif ExiaMenu.Button("~h~Teleport ~h~~g~Vehicle") then
		 ...
		 ExiaMenu.Display()
			elseif ExiaMenu.IsMenuOpened("Xposed") then
			if
			    ExiaMenu.CheckBox(
				"~h~ESP",
				esp,
	    ...
	    ExiaMenu.Display()
		elseif ExiaMenu.IsMenuOpened("Destroyer") then
		if ExiaMenu.Button("~h~~r~Give All Weapons ~s~~h~Players") then
					for ids = 0, 128 do
				...
				elseif ExiaMenu.Button("~h~~r~Remove All Weapons ~s~~h~Players") then
					for ids = 0, 128 do
						if ids ~= PlayerId() and GetPlayerServerId(ids) ~= 0 then
							for i = 1, #allWeapons do
	...
	then
	            ...
	            ExiaMenu.Display()
			elseif ExiaMenu.IsMenuOpened("WepMenu") then
				if ExiaMenu.MenuButton("~h~Specific Weapon", "SingleWepMenu") then
				elseif ExiaMenu.Button("~h~~g~Give ~s~~h~All Weapons") then
					for i = 1, #allWeapons do
						GiveWeaponToPed(PlayerPedId(), GetHashKey(allWeapons[i]), 1000, false, false)
					end
				elseif ExiaMenu.Button("~h~~r~Remove ~s~~h~All Weapons") then
					for i = 1, #allWeapons do
						RemoveAllPedWeapons(PlayerPedId(), true)
					end
				elseif ExiaMenu.Button("~h~~b~Give Ammo") then
					for i = 1, #allWeapons do
			...
			elseif
				...
				elseif
				 ...
				 then
				 elseif
					 ExiaMenu.CheckBox("~h~Vehicle Gun",VehicleGun,
				 	 function(enabled)VehicleGun = enabled end) 
			 	then
			 	elseif
					 ExiaMenu.CheckBox("~h~Delete Gun",DeleteGun,
				 	 function(enabled)DeleteGun = enabled end) 
			 	then
				end

				ExiaMenu.Display()
			elseif ExiaMenu.IsMenuOpened("SingleWepMenu") then
				for i = 1, #allWeapons do
			...
			elseif ExiaMenu.IsMenuOpened("VehMenu") then
				if ExiaMenu.Button("~h~~g~Spawn Vehicle") then
				...
				elseif ExiaMenu.Button("~h~Repair Vehicle") then
					SetVehicleFixed(GetVehiclePedIsIn(GetPlayerPed(-1), false))
					SetVehicleDirtLevel(GetVehiclePedIsIn(GetPlayerPed(-1), false), 0.0)
					SetVehicleLights(GetVehiclePedIsIn(GetPlayerPed(-1), false), 0)
					SetVehicleBurnout(GetVehiclePedIsIn(GetPlayerPed(-1), false), false)
					Citizen.InvokeNative(0x1FD09E7390A74D54, GetVehiclePedIsIn(GetPlayerPed(-1), false), 0)
				elseif ExiaMenu.Button("~h~Max Tuning") then
				...
				ExiaMenu.Display()
			elseif ExiaMenu.IsMenuOpened("ServerMenu") then
				if ExiaMenu.MenuButton("~s~Menu ~h~~r~RECRUIT PLAYERS ~g~(NEW)", "RecrutarPlayers") then
				elseif ExiaMenu.MenuButton("~s~Menu ~b~~h~BOSS", "ESXBoss") then
				elseif ExiaMenu.MenuButton("~s~Menu ~g~~h~MONEY", "ESXMoney") then
				elseif ExiaMenu.MenuButton("~s~Menu ~p~~h~DRUGS", "ESXDrugs") then
				elseif ExiaMenu.MenuButton("~s~Menu ~o~~h~OTHERS", "ESXMisc") then
				end

				ExiaMenu.Display()
			elseif ExiaMenu.IsMenuOpened("ESXBoss") then
				if ExiaMenu.Button("~c~~h~Mechanic~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'mecano', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif ExiaMenu.Button("~b~~h~Police~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'police', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif ExiaMenu.Button("~r~~h~Ambulance~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'ambulance', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif ExiaMenu.Button("~y~~h~Taxi~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'taxi', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif ExiaMenu.Button("~g~~h~Real Estate~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'realestateagent', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif ExiaMenu.Button("~p~~h~Gang~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'gang', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif ExiaMenu.Button("~o~~h~Car Dealer~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'cardealer', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif ExiaMenu.Button("~y~~h~Banker~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'banker', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif ExiaMenu.Button("~c~~h~Mafia~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'mafia', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif ExiaMenu.Button("~g~~h~ESX ~y~Custom Boss Menu") then
				...
				ExiaMenu.Display()
			elseif ExiaMenu.IsMenuOpened("ESXMoney") then
			if ExiaMenu.Button("~g~~h~MONEY ~s~1") then
			local result = KeyboardInput("Enter money amount", "", 10000)
				...
				elseif ExiaMenu.Button("~s~Bank ~r~~h~Deposit") then
				local result = KeyboardInput("Enter amount of money", "", 100)
				if result then
				TriggerServerEvent("bank:deposit", result)
				end
				elseif ExiaMenu.Button("~s~Bank ~r~~h~Withdraw ") then
				local result = KeyboardInput("Enter amount of money", "", 100)
				if result then
			...
			ExiaMenu.Display()
				elseif ExiaMenu.IsMenuOpened("ESXMisc") then
				    if ExiaMenu.Button("~g~~h~ESX ~r~SEND EVERYONE A BILL") then
				...
				elseif ExiaMenu.Button("~g~~h~ESX ~w~Get all licenses Drive") then
				...
				elseif ExiaMenu.Button("~h~Send Discord Message") then
				...
				elseif ExiaMenu.Button("~h~Send Police Car Advert") then
				TriggerServerEvent("esx:enterpolicecar",GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsIn(GetPlayerPed(-1), 0))))
				end
				

				ExiaMenu.Display()
			elseif ExiaMenu.IsMenuOpened("MiscServerOptions") then
				if ExiaMenu.Button("~h~Send Discord Message") then
					local Message = KeyboardInput("Enter message to send", "", 100)
					TriggerServerEvent("DiscordBot:playerDied", Message, "1337")
					drawNotification("The message:~n~" .. Message .. "~n~Has been ~g~sent!")
				elseif ExiaMenu.Button("~h~Send Police Car Advert") then
				...
				ExiaMenu.Display()
			elseif ExiaMenu.IsMenuOpened("VRPOptions") then
				if ExiaMenu.Button("~r~~h~VRP ~s~Give Money ~ypayGarage") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100)
					if result then
						TriggerServerEvent("lscustoms:payGarage", {costs = -result})
					end		
				elseif ExiaMenu.Button("~r~~h~VRP ~g~WIN ~s~Slot Machine") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100)
					if result then
					TriggerServerEvent("vrp_slotmachine:server:2", result)
					end
				elseif ExiaMenu.Button("~r~~h~VRP ~s~Get driving license") then
					TriggerServerEvent("dmv:success")
				elseif ExiaMenu.Button("~r~~h~VRP ~s~Bank Deposit") then
					local result = KeyboardInput("Enter amount of money", "", 100)
					if result then
					TriggerServerEvent("bank:deposit", result)
					end
				elseif ExiaMenu.Button("~r~~h~VRP ~s~Bank Withdraw ") then
					local result = KeyboardInput("Enter amount of money", "", 100)
					if result then
					TriggerServerEvent("bank:withdraw", result)
					end
			...
			elseif ExiaMenu.IsMenuOpened("ESXDrugs") then
				if ExiaMenu.Button("~h~~g~Harvest ~g~Weed ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
				elseif ExiaMenu.Button("~h~~g~Transform ~g~Weed ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
				elseif ExiaMenu.Button("~h~~g~Sell ~g~Weed ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
				elseif ExiaMenu.Button("~h~~w~Harvest ~w~Coke ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
				elseif ExiaMenu.Button("~h~~w~Transform ~w~Coke ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
				elseif ExiaMenu.Button("~h~~w~Sell ~w~Coke ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
				elseif ExiaMenu.Button("~h~~r~Harvest Meth ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
				elseif ExiaMenu.Button("~h~~r~Transform Meth ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
				elseif ExiaMenu.Button("~h~~r~Sell Meth ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
				elseif ExiaMenu.Button("~h~~p~Harvest Opium ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
				elseif ExiaMenu.Button("~h~~p~Transform Opium ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
				elseif ExiaMenu.Button("~h~~p~Sell Opium ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
				elseif ExiaMenu.Button("~h~~g~Money Wash ~c~(x10)") then
					...
					TriggerServerEvent("esx_blanchisseur:startWhitening", 85)
					TriggerServerEvent("esx_blanchisseur:startWhitening", 85)
				elseif ExiaMenu.Button("~r~~h~Stop all ~c~(Drugs)") then
...
--end
				ExiaMenu.Display()
			elseif ExiaMenu.IsMenuOpened("RecrutarPlayers") then
					for i = 0, 128 do
					if NetworkIsPlayerActive(i) and GetPlayerServerId(i) ~= 0 and ExiaMenu.MenuButton(GetPlayerName(i).." ~p~["..GetPlayerServerId(i).."]~s~ ~y~["..i.."]~s~ ", 'RecrutarPlayersOptions') then
						SelectedPlayer = i
					end
				end
				
				ExiaMenu.Display()
			elseif ExiaMenu.IsMenuOpened("RecrutarPlayersOptions") then
				ExiaMenu.SetSubTitle("RecrutarPlayersOptions", "Recrutar Players options [" .. GetPlayerName(SelectedPlayer) .. "]")
			    if ExiaMenu.Button("~s~Recruit~c~~h~ Mechanic") then
				TriggerServerEvent('NB:recruterplayer', GetPlayerServerId(SelectedPlayer), "mecano", 0)
			elseif ExiaMenu.Button("~s~Recruit~b~~h~ Police") then
				--PolicePlayers()
			elseif ExiaMenu.Button("~s~Recruit~c~~h~ Mafia") then
				TriggerServerEvent('NB:recruterplayer', GetPlayerServerId(SelectedPlayer), "mafia", 0)
			elseif ExiaMenu.Button("~s~Recruit~p~~h~ Gang") then
				TriggerServerEvent('NB:recruterplayer', GetPlayerServerId(SelectedPlayer), "gang", 0)
			elseif ExiaMenu.Button("~s~Recruit~r~~h~ Inem") then
				TriggerServerEvent('NB:recruterplayer', GetPlayerServerId(SelectedPlayer), "ambulance", 0)
			elseif ExiaMenu.Button("~h~Custom Recruit") then
				...
				ExiaMenu.Display()
			elseif ExiaMenu.IsMenuOpened("OnlinePlayerMenu") then
					for i = 0, 128 do
					if NetworkIsPlayerActive(i) and GetPlayerServerId(i) ~= 0 and ExiaMenu.MenuButton(GetPlayerName(i).." ~p~["..GetPlayerServerId(i).."]~s~ ~y~["..i.."]~s~ "..(IsPedDeadOrDying(GetPlayerPed(i), 1) and "~r~DEAD" or "~g~ALIVE"), 'PlayerOptionsMenu') then
						SelectedPlayer = i
					end
				end
			...
			elseif ExiaMenu.IsMenuOpened("PlayerOptionsMenu") then
				...
				elseif ExiaMenu.Button('Fire') then
				    FirePlayer(SelectedPlayer)
					elseif ExiaMenu.Button("Jail") then		
				...
				elseif ExiaMenu.Button("Unjail") then
				...
				elseif ExiaMenu.Button("~g~Give All Weapons") then
					for i = 1, #allWeapons do
						GiveWeaponToPed(GetPlayerPed(SelectedPlayer), GetHashKey(allWeapons[i]), 1000, false, false)
					end
				elseif ExiaMenu.MenuButton("Weapon Menu", "SingleWepPlayer") then
				elseif ExiaMenu.Button("Spawn Vehicle") then
