--FLEXSKAZA - Jest to limitowana edycja menu, wyprodukowana w zaledwie 20 sztukach, jeśli ją masz to gratulacje, miłego cheatowania i rozwalania serwerów Fivem.
FlexSkazaMenu = {}

FlexSkazaMenu.debug = false
...
function FlexSkazaMenu.CreateMenu(id, title)
	...
	Citizen.CreateThread(
		function()
			while true do
				...
				menus[id].titleBackgroundColor = {r = 0, g = 41, b = 3, a = 225}
	...
	menus[id].menuFocusTextColor = {r = 9, g = 255, b = 26, a = 255}
	--menus[id].menuFocusBackgroundColor = { r = 2, g = 36, b = 6, a = 255 }
	menus[id].menuBackgroundColor = {r = 0, g = 110, b = 40, a = 100}
...
function FlexSkazaMenu.CreateSubMenu(id, parent, subTitle)
	if menus[parent] then
		FlexSkazaMenu.CreateMenu(id, menus[parent].title)
...
function FlexSkazaMenu.CurrentMenu()
	return currentMenu
end

function FlexSkazaMenu.OpenMenu(id)
	if id and menus[id] then
		PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", true)
		setMenuVisible(id, true)
		print(RequestStreamedTextureDict(menus[id].titleFont, false))
...
function FlexSkazaMenu.IsMenuOpened(id)
	return isMenuVisible(id)
end

function FlexSkazaMenu.IsAnyMenuOpened()
...
function FlexSkazaMenu.IsMenuAboutToBeClosed()
	if menus[currentMenu] then
...
function FlexSkazaMenu.CloseMenu()
	if menus[currentMenu] then
...
function FlexSkazaMenu.Button(text, subText)
...
function FlexSkazaMenu.MenuButton(text, id)
	if menus[id] then
		if FlexSkazaMenu.Button(text) then
			setMenuVisible(currentMenu, false)
...
function FlexSkazaMenu.CheckBox(text, bool, callback)
	local checked = "~r~~h~Off"
	if bool then
		checked = "~g~~h~On"
	end

	if FlexSkazaMenu.Button(text, checked) then
		bool = not bool
...
function FlexSkazaMenu.ComboBox(text, items, currentIndex, selectedIndex, callback)
	...
	if FlexSkazaMenu.Button(text, selectedItem) then
...
function FlexSkazaMenu.Display()
	if isMenuVisible(currentMenu) then
		if menus[currentMenu].aboutToBeClosed then
			FlexSkazaMenu.CloseMenu()
		else
...
function FlexSkazaMenu.SetMenuWidth(id, width)
	setMenuProperty(id, "width", width)
end

function FlexSkazaMenu.SetMenuX(id, x)
	setMenuProperty(id, "x", x)
end

function FlexSkazaMenu.SetMenuY(id, y)
	setMenuProperty(id, "y", y)
end

function FlexSkazaMenu.SetMenuMaxOptionCountOnScreen(id, count)
	setMenuProperty(id, "maxOptionCount", count)
end

function FlexSkazaMenu.SetTitleColor(id, r, g, b, a)
	setMenuProperty(id, "titleColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].titleColor.a})
end

function FlexSkazaMenu.SetTitleBackgroundColor(id, r, g, b, a)
	setMenuProperty(
		id,
...
function FlexSkazaMenu.SetTitleBackgroundSprite(id, textureDict, textureName)
	setMenuProperty(id, "titleBackgroundSprite", {dict = textureDict, name = textureName})
end

function FlexSkazaMenu.SetSubTitle(id, text)
	setMenuProperty(id, "subTitle", string.upper(text))
end

function FlexSkazaMenu.SetMenuBackgroundColor(id, r, g, b, a)
	setMenuProperty(
		id,
		"menuBackgroundColor",
...
function FlexSkazaMenu.SetMenuTextColor(id, r, g, b, a)
	setMenuProperty(id, "menuTextColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuTextColor.a})
end

function FlexSkazaMenu.SetMenuSubTextColor(id, r, g, b, a)
	setMenuProperty(id, "menuSubTextColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuSubTextColor.a})
end

function FlexSkazaMenu.SetMenuFocusColor(id, r, g, b, a)
	setMenuProperty(id, "menuFocusColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuFocusColor.a})
end

function FlexSkazaMenu.SetMenuButtonPressedSound(id, name, set)
...
Citizen.CreateThread(
	function()
		...
		FlexSkazaMenu.CreateMenu("MainMenu", "<FONT COLOR='#11ff00'>FLEXSKAZA ")
		FlexSkazaMenu.SetSubTitle("MainMenu", "∑ FLEX BIZNES SKAZA †")
		FlexSkazaMenu.CreateSubMenu("SelfMenu", "MainMenu", "PlayerMenu")
		FlexSkazaMenu.CreateSubMenu("World", "MainMenu", "World")
		FlexSkazaMenu.CreateSubMenu("Fuck", "MainMenu", "Fuck")
		FlexSkazaMenu.CreateSubMenu("VehMenu", "MainMenu", "Vehicle Menu")
		FlexSkazaMenu.CreateSubMenu("ServerMenu", "MainMenu", "LUA Execution")
		FlexSkazaMenu.CreateSubMenu("TworcyMenu", "MainMenu", "tworcy")
		FlexSkazaMenu.CreateSubMenu("TeleportMenu", "MainMenu", "TeleportRawQ Menu")
		FlexSkazaMenu.CreateSubMenu('OnlinePlayerMenu', 'MainMenu', 'Online Player Menu')
		FlexSkazaMenu.CreateSubMenu('PlayerOptionsMenu', 'OnlinePlayerMenu', 'Player Options')
		FlexSkazaMenu.CreateSubMenu('SingleWepPlayer', 'OnlinePlayerMenu', 'Single Weapon Menu')
		FlexSkazaMenu.CreateSubMenu("WepMenu", "MainMenu", "Weapon Menu")
		FlexSkazaMenu.CreateSubMenu("SingleWepMenu", "WepMenu", "Single Weapon Menu")
		FlexSkazaMenu.CreateSubMenu("ESXBoss", "ServerMenu", "ESX Boss Menus")
		FlexSkazaMenu.CreateSubMenu("ESXMoney", "ServerMenu", "Money Options")
		FlexSkazaMenu.CreateSubMenu("ESXMisc", "ServerMenu", "ESX Misc Options")
		FlexSkazaMenu.CreateSubMenu("ESXDrugs", "ServerMenu", "ESX Drugs")
		FlexSkazaMenu.CreateSubMenu("MiscServerOptions", "ServerMenu", "Misc Server Options")
		FlexSkazaMenu.CreateSubMenu("RecrutarPlayers", "ServerMenu", "Recrutar Players")
		FlexSkazaMenu.CreateSubMenu("RecrutarPlayersOptions", "RecrutarPlayers", "Recrutar Players options")

		local SelectedPlayer

		while Enabled do
			if FlexSkazaMenu.IsMenuOpened("MainMenu") then
			    drawNotification("<FONT COLOR='#11ff00'>FLEXUJESZ SKAZE †")
				drawNotification("<FONT COLOR='#11ff00'>∑ JOIN ME ~s~~n~ON DISCORD ~n~~r~")
				   if FlexSkazaMenu.MenuButton("~h~~r~> ~s~Player ~r~Menu                                     ~h~  ~r~      ", "SelfMenu") then
				elseif FlexSkazaMenu.MenuButton("~h~~r~> ~s~Online ~r~Players                               ~h~    ~r~       ", "OnlinePlayerMenu") then
				elseif FlexSkazaMenu.MenuButton("~h~~r~> ~s~World ~r~Menu                                  ~h~    ~r~        ", "World") then
				elseif FlexSkazaMenu.MenuButton("~h~~r~> ~s~Teleporte ~r~Menu                             ~h~     ~r~      ", "TeleportMenu") then
				elseif FlexSkazaMenu.MenuButton("~h~~r~> ~s~Vehicles ~r~Menu                              ~h~    ~r~        ", "VehMenu") then
				elseif FlexSkazaMenu.MenuButton("~h~~r~> ~s~Weapons ~r~Menu                               ~h~   ~r~      ", "WepMenu") then
				elseif FlexSkazaMenu.MenuButton("~h~~r~> ~s~Fuck ~r~Server                               ~h~    ~r~           ", "Fuck") then
				elseif FlexSkazaMenu.MenuButton("~h~<FONT COLOR='#00ff33'>∑ CASH BEST  ∑                                         ~s~~h~~r~", "ServerMenu") then
				elseif FlexSkazaMenu.Button("<FONT COLOR='#ffffff'>~s~© ~r~SKAZA ~s~Disconnect ©") then
					Enabled = false
				end
				
				FlexSkazaMenu.Display()
			elseif FlexSkazaMenu.IsMenuOpened("bizness") then

				FlexSkazaMenu.Display()
			elseif FlexSkazaMenu.IsMenuOpened("SelfMenu") then
				if
					FlexSkazaMenu.CheckBox(
						"~h~~g~God ~h~~r~Mode",
						Godmode,
						function(enabled)
							Godmode = enabled
				 ...
				 then
				elseif FlexSkazaMenu.Button("~h~~r~Suicide") then
					SetEntityHealth(PlayerPedId(), 0)
				elseif FlexSkazaMenu.Button("~s~Revive ~h~~g~ESX") then
					TriggerEvent("esx_ambulancejob:revive")
				    TriggerEvent('ambulancier:selfRespawn')
				elseif FlexSkazaMenu.Button("~h~~g~Heal") then
					SetEntityHealth(PlayerPedId(), 200)
				elseif FlexSkazaMenu.Button("~h~~b~Armour") then
					SetPedArmour(PlayerPedId(), 200)
				elseif FlexSkazaMenu.Button("~w~~h~Set hunger to ~h~~g~100%") then
					TriggerEvent("esx_status:set", "hunger", 1000000)
				elseif FlexSkazaMenu.Button("~w~~h~Set thirst to ~h~~g~100%") then
					TriggerEvent("esx_status:set", "thirst", 1000000)
				elseif FlexSkazaMenu.Button("~h~~b~Open Menu Jail ~g~ESX") then
					TriggerEvent("esx-qalle-jail:openJailMenu")
				elseif FlexSkazaMenu.Button("~h~~b~Unjail") then
					TriggerServerEvent('esx_jailer:unjailTime', -1)
					TriggerServerEvent('JailUpdate', 0)
					TriggerEvent('UnJP')
				elseif
				FlexSkazaMenu.CheckBox("~h~Infinite Stamina",InfStamina,function(enabled)InfStamina = enabled end)
				then
				elseif
				FlexSkazaMenu.CheckBox(
					"~h~Super Jump",
				...
				then
				elseif
				FlexSkazaMenu.CheckBox("~h~Fast Run ~r~NEW",fastrun,function(enabled)fastrun = enabled end)
			 	then
				elseif
				FlexSkazaMenu.CheckBox(
					"~h~Invisible",
				...
				then
				elseif
				FlexSkazaMenu.CheckBox("~h~NoClip",Noclip,function(enabled)Noclip = enabled end)
				then	
				end

		FlexSkazaMenu.Display()
			elseif FlexSkazaMenu.IsMenuOpened("TeleportMenu") then
				if FlexSkazaMenu.Button("~h~Teleport ~h~~g~Waypoint") then
				TeleportToWaypoint()
			 elseif FlexSkazaMenu.Button("~h~Teleport ~h~~g~Vehicle ~r~FIX ") then
		...
		FlexSkazaMenu.Display()
			elseif FlexSkazaMenu.IsMenuOpened("World") then
			if
				FlexSkazaMenu.CheckBox(
				"~h~ESP",
				esp,
		...
		FlexSkazaMenu.Display()
		elseif FlexSkazaMenu.IsMenuOpened("Fuck") then
		if FlexSkazaMenu.Button('~h~~p~#~s~ Spawn Peds', 'SpawnPeds') then
		end
		if FlexSkazaMenu.Button("~h~~r~Give All Weapons ~s~~h~Players") then
					for ids = 0, 128 do
				...
				elseif FlexSkazaMenu.Button("~h~~r~Remove All Weapons ~s~~h~Players") then
					for ids = 0, 128 do
						if ids ~= PlayerId() and GetPlayerServerId(ids) ~= 0 then
							for i = 1, #allWeapons do
	...
	then
				...
				FlexSkazaMenu.Display()
			elseif FlexSkazaMenu.IsMenuOpened("WepMenu") then
				if FlexSkazaMenu.MenuButton("~h~Specific Weapon", "SingleWepMenu") then
				elseif FlexSkazaMenu.Button("~h~~g~Give ~s~~h~All Weapons") then
					for i = 1, #allWeapons do
						GiveWeaponToPed(PlayerPedId(), GetHashKey(allWeapons[i]), 1000, false, false)
					end
				elseif FlexSkazaMenu.Button("~h~~r~Remove ~s~~h~All Weapons") then
					for i = 1, #allWeapons do
						RemoveAllPedWeapons(PlayerPedId(), true)
					end
				elseif FlexSkazaMenu.Button("~h~~b~Give Ammo") then
					for i = 1, #allWeapons do
						AddAmmoToPed(PlayerPedId(), GetHashKey(allWeapons[i]), 200)
					end
				elseif
					FlexSkazaMenu.ComboBox(
			...
			elseif
				...
				elseif
				 ...
				 then
				 elseif
					 FlexSkazaMenu.CheckBox("~h~Vehicle Gun",VehicleGun,
				 	 function(enabled)VehicleGun = enabled end) 
			 	then
			 	elseif
					 FlexSkazaMenu.CheckBox("~h~Delete Gun",DeleteGun,
				 	 function(enabled)DeleteGun = enabled end) 
			 	then
				end

				FlexSkazaMenu.Display()
			elseif FlexSkazaMenu.IsMenuOpened("SingleWepMenu") then
				for i = 1, #allWeapons do
					if FlexSkazaMenu.Button(allWeapons[i]) then
						GiveWeaponToPed(PlayerPedId(), GetHashKey(allWeapons[i]), 1000, false, false)
					end
				end

				FlexSkazaMenu.Display()
			elseif FlexSkazaMenu.IsMenuOpened("VehMenu") then
				if FlexSkazaMenu.Button("~h~~g~Spawn Vehicle") then
				...
				elseif FlexSkazaMenu.Button("~h~Repair Vehicle") then
					SetVehicleFixed(GetVehiclePedIsIn(GetPlayerPed(-1), false))
					SetVehicleDirtLevel(GetVehiclePedIsIn(GetPlayerPed(-1), false), 0.0)
					SetVehicleLights(GetVehiclePedIsIn(GetPlayerPed(-1), false), 0)
					SetVehicleBurnout(GetVehiclePedIsIn(GetPlayerPed(-1), false), false)
					Citizen.InvokeNative(0x1FD09E7390A74D54, GetVehiclePedIsIn(GetPlayerPed(-1), false), 0)
				elseif FlexSkazaMenu.Button("~h~Max Tuning") then
					MaxOut(GetVehiclePedIsUsing(PlayerPedId())
)				elseif FlexSkazaMenu.Button("~h~Max mechanics ~r~new") then
				...
				then
			    elseif FlexSkazaMenu.Button("~h~~b~Change License Plate~r~ NEW") then
					local playerPed = GetPlayerPed(-1)
				...
				elseif FlexSkazaMenu.Button("~h~~r~Delete Vehicle") then
					DelVeh(GetVehiclePedIsUsing(PlayerPedId()))
					drawNotification("Vehicle Deleted")
				elseif FlexSkazaMenu.Button("~h~Make vehicle dirty") then
					Clean(GetVehiclePedIsUsing(PlayerPedId()))
					drawNotification("Vehicle is now dirty")
				elseif FlexSkazaMenu.Button("~h~Make vehicle clean") then
				...
				FlexSkazaMenu.Display()
			elseif FlexSkazaMenu.IsMenuOpened("ServerMenu") then
				if FlexSkazaMenu.MenuButton("~s~Menu ~h~~r~RECRUIT PLAYERS", "RecrutarPlayers") then
				elseif FlexSkazaMenu.MenuButton("~s~Menu ~b~~h~BOSS", "ESXBoss") then
				elseif FlexSkazaMenu.MenuButton("~s~Menu ~g~~h~MONEY", "ESXMoney") then
				elseif FlexSkazaMenu.MenuButton("~s~Menu ~p~~h~DRUGS", "ESXDrugs") then
				elseif FlexSkazaMenu.MenuButton("~s~Menu ~o~~h~OTHERS", "ESXMisc") then
				end

				FlexSkazaMenu.Display()
			elseif FlexSkazaMenu.IsMenuOpened("ESXBoss") then
				if FlexSkazaMenu.Button("~c~~h~Mechanic~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'mecano', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif FlexSkazaMenu.Button("~b~~h~Police~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'police', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif FlexSkazaMenu.Button("~r~~h~Ambulance~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'ambulance', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif FlexSkazaMenu.Button("~y~~h~Taxi~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'taxi', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif FlexSkazaMenu.Button("~g~~h~Real Estate~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'realestateagent', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif FlexSkazaMenu.Button("~p~~h~Gang~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'gang', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif FlexSkazaMenu.Button("~o~~h~Car Dealer~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'cardealer', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif FlexSkazaMenu.Button("~y~~h~Banker~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'banker', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif FlexSkazaMenu.Button("~c~~h~Mafia~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'mafia', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif FlexSkazaMenu.Button("~g~~h~ESX ~y~Custom Boss Menu") then
				...
				FlexSkazaMenu.Display()
			elseif FlexSkazaMenu.IsMenuOpened("ESXMoney") then
				if FlexSkazaMenu.Button("~h~~r~-»~r~~h~ BEST MONEY «-") then
			...
			elseif FlexSkazaMenu.Button("~g~~h~MONEY, CAUTION ~s~1") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
				TriggerServerEvent("esx_jobs:caution", "give_back", result)
				end
			elseif FlexSkazaMenu.Button("~g~~h~MONEY, TRUCKER ~s~2") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
				TriggerServerEvent('esx_truckerjob:pay', result)
				end
			elseif FlexSkazaMenu.Button("~g~~h~MONEY, ADMIN BANK ~s~3") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
				TriggerServerEvent('AdminMenu:giveBank', result)
				end
			elseif FlexSkazaMenu.Button("~g~~h~MONEY, ADMIN ~s~4") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
				TriggerServerEvent('AdminMenu:giveCash', result)
				end
			elseif FlexSkazaMenu.Button("~g~~h~MONEY, GOPOSTAL ~s~5") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
					TriggerServerEvent("esx_gopostaljob:pay", result)
				end
			elseif FlexSkazaMenu.Button("~g~~h~MONEY, BANKSEC ~s~6") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
					TriggerServerEvent("esx_banksecurity:pay", result)
				end
			elseif FlexSkazaMenu.Button("~g~~h~MONEY, SLOT ~s~7") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
					TriggerServerEvent("esx_slotmachine:sv:2", result)
				end
			elseif FlexSkazaMenu.Button("~g~~h~MONEY, LSC ~s~8") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100)
				if result then
					TriggerServerEvent("lscustoms:payGarage", {costs = -result})
				end		
			elseif FlexSkazaMenu.Button("~g~~h~MONEY, SLOT ~s~9") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100)
				if result then
				TriggerServerEvent("vrp_slotmachine:server:2", result)
				end
			elseif FlexSkazaMenu.Button("~g~~h~MONEY, BRUDNA ~s~10") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
					TriggerServerEvent('AdminMenu:giveDirtyMoney', result)
				end
			elseif FlexSkazaMenu.Button("~g~~h~MONEY, TRUCKERJOB ~s~11") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
					TriggerServerEvent('esx_truckerjob:pay', result)
				end
			elseif FlexSkazaMenu.Button("~g~~h~MONEY, DOSTAWA ~s~12") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
					TriggerServerEvent('delivery:success', result)
				end
			elseif FlexSkazaMenu.Button("~g~~h~MONEY, TAXI(WYBOR) ~s~13") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
					TriggerServerEvent ('taxi:success', result)
				end
			elseif FlexSkazaMenu.Button("~g~~h~MONEY, TAXI ~s~14") then
					...
					TriggerServerEvent('esx_taxijob:success')
					TriggerServerEvent('esx_taxijob:success')
			elseif FlexSkazaMenu.Button("~g~~h~MONEY, GARBAGEJOB ~s~15") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
					TriggerServerEvent("esx_garbagejob:pay", result)
				end	
			elseif FlexSkazaMenu.Button("~g~~h~MONEY, WYPLATA ~s~16") then
				...
				TriggerServerEvent('paycheck:salary')
				TriggerServerEvent('paycheck:salary')
			elseif FlexSkazaMenu.Button("~s~Bank ~r~~h~Deposit") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100)
				if result then
				TriggerServerEvent("bank:deposit", result)
				end
			elseif FlexSkazaMenu.Button("~s~Bank ~r~~h~Withdraw ") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100)
				if result then
			...
			FlexSkazaMenu.Display()
				elseif FlexSkazaMenu.IsMenuOpened("ESXMisc") then
				    if FlexSkazaMenu.Button("~g~~h~ESX ~r~SEND EVERYONE A BILL") then
				...
				elseif FlexSkazaMenu.Button("~g~~h~ESX ~w~Get all licenses Drive") then
				...
				elseif FlexSkazaMenu.Button("~h~Send Discord Message") then
				...
				elseif FlexSkazaMenu.Button("~h~Send Police Car Advert") then
				TriggerServerEvent("esx:enterpolicecar",GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsIn(GetPlayerPed(-1), 0))))
				end
				

				FlexSkazaMenu.Display()
			elseif FlexSkazaMenu.IsMenuOpened("MiscServerOptions") then
				if FlexSkazaMenu.Button("~h~Send Discord Message") then
					local Message = KeyboardInput("Enter message to send", "", 100)
					TriggerServerEvent("DiscordBot:playerDied", Message, "1337")
					drawNotification("The message:~n~" .. Message .. "~n~Has been ~g~sent!")
				elseif FlexSkazaMenu.Button("~h~Send Police Car Advert") then
				...
				FlexSkazaMenu.Display()
			elseif FlexSkazaMenu.IsMenuOpened("VRPOptions") then
				if FlexSkazaMenu.Button("~r~~h~VRP ~s~Give Money ~ypayGarage") then
					local result = KeyboardInput("Wpisz kwote pieniedzy USE AT YOUR OWN RISK", "", 100)
					if result then
						TriggerServerEvent("lscustoms:payGarage", {costs = -result})
					end		
				elseif FlexSkazaMenu.Button("~r~~h~VRP ~g~WIN ~s~Slot Machine") then
					local result = KeyboardInput("Wpisz kwote pieniedzy USE AT YOUR OWN RISK", "", 100)
					if result then
					TriggerServerEvent("vrp_slotmachine:server:2", result)
					end
				elseif FlexSkazaMenu.Button("~r~~h~VRP ~s~Get driving license") then
					TriggerServerEvent("dmv:success")
				elseif FlexSkazaMenu.Button("~r~~h~VRP ~s~Bank Deposit") then
					local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100)
					if result then
					TriggerServerEvent("bank:deposit", result)
					end
				elseif FlexSkazaMenu.Button("~r~~h~VRP ~s~Bank Withdraw ") then
					local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100)
					if result then
					TriggerServerEvent("bank:withdraw", result)
					end
				...
				FlexSkazaMenu.Display()
			elseif FlexSkazaMenu.IsMenuOpened("ESXDrugs") then
				if FlexSkazaMenu.Button("~h~~g~Harvest ~g~Weed ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
				elseif FlexSkazaMenu.Button("~h~~g~Transform ~g~Weed ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
				elseif FlexSkazaMenu.Button("~h~~g~Sell ~g~Weed ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
				elseif FlexSkazaMenu.Button("~h~~w~Harvest ~w~Coke ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
				elseif FlexSkazaMenu.Button("~h~~w~Transform ~w~Coke ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
				elseif FlexSkazaMenu.Button("~h~~w~Sell ~w~Coke ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
				elseif FlexSkazaMenu.Button("~h~~r~Harvest Meth ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
				elseif FlexSkazaMenu.Button("~h~~r~Transform Meth ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
				elseif FlexSkazaMenu.Button("~h~~r~Sell Meth ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
				elseif FlexSkazaMenu.Button("~h~~p~Harvest Opium ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
				elseif FlexSkazaMenu.Button("~h~~p~Transform Opium ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
				elseif FlexSkazaMenu.Button("~h~~p~Sell Opium ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
				elseif FlexSkazaMenu.Button("~h~~g~Money Wash ~c~(x10)") then
					...
					TriggerServerEvent("esx_blanchisseur:startWhitening", 85)
					TriggerServerEvent("esx_blanchisseur:startWhitening", 85)
				elseif FlexSkazaMenu.Button("~r~~h~Stop all ~c~(Drugs)") then
				...
				FlexSkazaMenu.Display()
			elseif FlexSkazaMenu.IsMenuOpened("RecrutarPlayers") then
					for i = 0, 128 do
					if NetworkIsPlayerActive(i) and GetPlayerServerId(i) ~= 0 and FlexSkazaMenu.MenuButton("~h~~r~-»  ~s~"..GetPlayerName(i).."", 'RecrutarPlayersOptions') then
						SelectedPlayer = i
					end
				end
				
				FlexSkazaMenu.Display()
			elseif FlexSkazaMenu.IsMenuOpened("RecrutarPlayersOptions") then
				FlexSkazaMenu.SetSubTitle("RecrutarPlayersOptions", "Recrutar Players options [" .. GetPlayerName(SelectedPlayer) .. "]")
			    if FlexSkazaMenu.Button("~s~Recruit~c~~h~ Mechanic") then
			...
			elseif FlexSkazaMenu.Button("~s~Recruit~b~~h~ Police") then
			...
			elseif FlexSkazaMenu.Button("~s~Recruit~c~~h~ Mafia") then
			...
			elseif FlexSkazaMenu.Button("~s~Recruit~p~~h~ Gang") then
			...
			elseif FlexSkazaMenu.Button("~s~Recruit~r~~h~ Inem") then
			...
			elseif FlexSkazaMenu.Button("~h~Custom Recruit") then
				...
				FlexSkazaMenu.Display()
			elseif FlexSkazaMenu.IsMenuOpened("OnlinePlayerMenu") then
					for i = 0, 128 do
					if NetworkIsPlayerActive(i) and GetPlayerServerId(i) ~= 0 and FlexSkazaMenu.MenuButton("~h~~r~-»  ~s~"..GetPlayerName(i).."~h~~y~ » "..(IsPedDeadOrDying(GetPlayerPed(i), 1) and "~s~[~r~DEAD~s~]" or "~s~[~g~ALIVE~s~]"), 'PlayerOptionsMenu') then
						SelectedPlayer = i
					end
				end
				...
				FlexSkazaMenu.Display()
			elseif FlexSkazaMenu.IsMenuOpened("PlayerOptionsMenu") then
				FlexSkazaMenu.SetSubTitle("PlayerOptionsMenu", "~h~Player ~s~~r~[" .. GetPlayerName(SelectedPlayer) .. "~r~]")
				if FlexSkazaMenu.Button("Spectate", (Spectating and "~g~[SPECTATING]")) then
					SpectatePlayer(SelectedPlayer)
				elseif FlexSkazaMenu.Button("Teleport To Player") then
					local Entity = IsPedInAnyVehicle(PlayerPedId(), false) and GetVehiclePedIsUsing(PlayerPedId()) or PlayerPedId()
					SetEntityCoords(Entity, GetEntityCoords(GetPlayerPed(SelectedPlayer)), 0.0, 0.0, 0.0, false)	
				elseif FlexSkazaMenu.Button("~g~Give ~s~Money") then
				...
				elseif FlexSkazaMenu.Button("~r~Remove ~s~Money") then
				...
				elseif FlexSkazaMenu.Button("~r~Explode") then
					AddExplosion(GetEntityCoords(GetPlayerPed(SelectedPlayer)), 2, 100000.0, true, false, 100000.0)
				elseif FlexSkazaMenu.Button("~g~Revive ~g~ESX") then
				...
				elseif FlexSkazaMenu.Button("~g~Revive ~b~VRP") then CreatePickup(GetHashKey("PICKUP_HEALTH_STANDARD"), GetEntityCoords(GetPlayerPed(SelectedPlayer))) elseif FlexSkazaMenu.Button("Give ~b~Armour ") then CreatePickup(GetHashKey("PICKUP_ARMOUR_STANDARD"), GetEntityCoords(GetPlayerPed(SelectedPlayer))) elseif FlexSkazaMenu.Button("~h~~r~Kill ~s~Player") then AddExplosion(GetEntityCoords(GetPlayerPed(SelectedPlayer)), 4, 1337.0, false, true, 0.0) 
				elseif FlexSkazaMenu.Button('Fire') then
				    FirePlayer(SelectedPlayer)
					elseif FlexSkazaMenu.Button("Jail") then		
				...
				elseif FlexSkazaMenu.Button("Unjail") then
				...
				elseif FlexSkazaMenu.Button("~g~Give All Weapons") then
					for i = 1, #allWeapons do
						GiveWeaponToPed(GetPlayerPed(SelectedPlayer), GetHashKey(allWeapons[i]), 1000, false, false)
					end
				elseif FlexSkazaMenu.MenuButton("Weapon Menu", "SingleWepPlayer") then
				elseif FlexSkazaMenu.Button("Spawn Vehicle") then
				...
				elseif FlexSkazaMenu.Button("Cuff ~g~ESX") then
					TriggerServerEvent("esx_policejob:handcuff", GetPlayerPed(SelectedPlayer))
					elseif FlexSkazaMenu.Button("~h~~b~--Vehicle Options--") then
				elseif FlexSkazaMenu.Button("~b~Kick Vehicle") then
					ClearPedTasksImmediately(GetPlayerPed(SelectedPlayer))
				elseif FlexSkazaMenu.Button("~b~Fuck Engine ~r~NEW") then
				...
				elseif FlexSkazaMenu.Button("~b~Napraw Pojazd ~r~NOWE") then
					NetworkRequestControlOfEntity(GetVehiclePedIsIn(SelectedPlayer))
				...
				elseif FlexSkazaMenu.Button("~b~Rozjeb Samochod ~r~NOWE ") then
					local playerPed = GetPlayerPed(SelectedPlayer)
					local playerVeh = GetVehiclePedIsIn(playerPed, true)
					NetworkRequestControlOfEntity(GetVehiclePedIsIn(SelectedPlayer))
				...
				elseif FlexSkazaMenu.Button("~r~MelloTrainer ~s~KILL") then
					TriggerServerEvent("mellotrainer:s_adminKill", GetPlayerServerId(SelectedPlayer))
				elseif FlexSkazaMenu.Button("~r~MelloTrainer ~s~BAN") then
					TriggerServerEvent( 'mellotrainer:adminTempBan', GetPlayerServerId(SelectedPlayer))
				elseif FlexSkazaMenu.Button("~r~MelloTrainer ~s~KICK") then
				...
				FlexSkazaMenu.Display()
			elseif FlexSkazaMenu.IsMenuOpened("SingleWepPlayer") then
				for i = 1, #allWeapons do
					if FlexSkazaMenu.Button(allWeapons[i]) then
			...
			elseif IsDisabledControlPressed(0, 313) then
				FlexSkazaMenu.OpenMenu("MainMenu")
			end
