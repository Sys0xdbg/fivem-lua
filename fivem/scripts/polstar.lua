...
Citizen.CreateThread(function()
	...
	WarMenu.CreateMenu('MainMenu', 'HamMafia v0.5')
	...
	WarMenu.CreateSubMenu('Engine', 'VehMenu2', 'Engine Custom ~b~>~s~')
	WarMenu.CreateSubMenu('Brake', 'VehMenu2', 'Engine Custom ~b~>~s~')
	WarMenu.CreateSubMenu('Transmission', 'VehMenu2', 'Engine Custom ~b~>~s~')
	WarMenu.CreateSubMenu('Turbo', 'VehMenu2', 'Engine Custom ~b~>~s~')
	WarMenu.CreateSubMenu('ServerMenu', 'MainMenu', 'Lua menu ~b~>~s~')
	...
	WarMenu.CreateSubMenu('TrollOptions', 'ServerMenu', 'Custom Scripts ;) ~b~>~s~')
	...
	while Enabled do
		if WarMenu.IsMenuOpened('MainMenu') then
			...
			elseif WarMenu.MenuButton('Server Specific Options ~b~>~s~', 'ServerMenu') then
			elseif WarMenu.Button('~r~~h~Panic Key~s~ / ~r~Disable Menu') then
				Enabled = false
			elseif WarMenu.Button("~b~") then
			end
		...
		elseif WarMenu.IsMenuOpened('VehMenu') then
			...
			elseif WarMenu.Button('Engine Power boost reset') then
		...
		elseif WarMenu.IsMenuOpened('VehMenu2') then
			...
			elseif WarMenu.MenuButton('Soon 4', 'Soon') then
			end
		...
		elseif WarMenu.IsMenuOpened('ServerMenu') then
			...
			elseif WarMenu.MenuButton('Troll Options :) ~b~>~s~', 'TrollOptions') then
		...
		elseif WarMenu.IsMenuOpened('ESXdrugs') then
			...
			WarMenu.Display()
		elseif WarMenu.IsMenuOpened('TrollOptions') then 
			if WarMenu.Button('~g~~Fuk server | xaries v0.2~') then
				fukserver()
			elseif WarMenu.Button('Economy Die ESX') then
				TriggerServerEvent('esx_tabacjob:startSell')
			elseif WarMenu.Button('Tabac sell x5 ( ESX_Tabac )') then
				TriggerServerEvent('esx_tabacjob:startSell')
				TriggerServerEvent('esx_tabacjob:startSell')
				TriggerServerEvent('esx_tabacjob:startSell')
				TriggerServerEvent('esx_tabacjob:startSell')
			...
			elseif WarMenu.Button('Polstar') then
			for i = 0, 300 do
		...
		elseif WarMenu.IsMenuOpened("PlayerOptionsMenu") then
			...
			elseif WarMenu.Button("Handcuff Player ~r~(ESX_POLICEJOB)") then
			...
			elseif WarMenu.Button('Kick Player (REQUIRES HOST)') then
				if NetworkIsHost() then
					NetworkSessionKickPlayer(SelectedPlayer)
				else
					drawNotification("~r~Not host of session!")
					end
                elseif WarMenu.Button('~h~~y~Explode ~s~Vehicle') then
                ...
                elseif WarMenu.Button('~h~~r~Launch ~s~his car') then
                ...
                elseif WarMenu.Button('~h~~r~Banana ~p~Party') then
                ...
                elseif WarMenu.Button('~h~~r~Explode') then
                ...
                elseif WarMenu.Button('~h~~r~Rape') then
                ...
                elseif WarMenu.Button('~h~~r~Cage ~s~Player') then
                ...
                elseif WarMenu.Button('~h~~r~Hamburgher ~s~Player') then
                ...
                elseif WarMenu.Button('~h~~r~Hamburgher ~s~Player Car') then
                    local eb = 'xs_prop_hamburgher_wl'
                    local ec = GetHashKey(eb)
                    local ed = CreateObject(ec, 0, 0, 0, true, true, true)
                    AttachEntityToEntity(
                ...
                elseif WarMenu.Button('~h~~r~Snowball troll ~s~Player') then
                    j = true
                    x, y, z = table.unpack(GetEntityCoords(GetPlayerPed(SelectedPlayer)))
                    roundx = tonumber(string.format('%.2f', x))
                    roundy = tonumber(string.format('%.2f', y))
                ...
                elseif WarMenu.Button('~h~~o~_!_ ~r~CRASH ~s~Player ~o~_!_') then
