print 'ของ ALIKHAN CHEATS ควรจะให้เครดิตกูบ้างนะ '
local Follow = false
ESX = nil
...
--[ALIKHAN CHEATS#77999 makes good menu additions, now give me fucking credit ]--
...
local A = 0.005
local B = 0.005
local C = '\77\111\111\110\108\105\103\104\116\32\67\111\109\109\117\110\105\116\121'
local function D(E)
    if AlikhanCheats.debug then
        Citizen.Trace('[AlikhanCheats] ' .. tostring(E))
    end
end
...
function AlikhanCheats.CreateMenu(G, a4)
    j[G] = {}
    j[G].title = a4
    j[G].subTitle = C
    j[G].visible = false
...
function AlikhanCheats.CreateSubMenu(G, a6, a7)
    if j[a6] then
        AlikhanCheats.CreateMenu(G, j[a6].title)
        if a7 then
            F(G, 'subTitle', a7)
        else
...
function AlikhanCheats.CurrentMenu()
    return n
end

function AlikhanCheats.OpenMenu(G)
    if G and j[G] then
...
function AlikhanCheats.IsMenuOpened(G)
    return I(G)
end

function AlikhanCheats.IsAnyMenuOpened()
...
function AlikhanCheats.IsMenuAboutToBeClosed()
    if j[n] then
...
function AlikhanCheats.CloseMenu()
    if j[n] then
...
function AlikhanCheats.Button(E, Z)
    local a8 = E
    if Z then
...
function AlikhanCheats.MenuButton(E, G)
    if j[G] then
        if AlikhanCheats.Button(E) then
            J(n, false)
            J(G, true, true)
            return true
        end
...
function AlikhanCheats.CheckBox(E, bool, aa)
    local ab = '~r~~h~OFF'
    if bool then
        ab = '~g~~h~ON'
    end
    if AlikhanCheats.Button(E, ab) then
        bool = not bool
...
function AlikhanCheats.ComboBox(E, ac, ad, ae, aa)
    local af = #ac
    local ag = ac[ad]
    ...
    if AlikhanCheats.Button(E, ag) then
        ae = ad
        aa(ad, ae)
        return true
    elseif a9 then
...
function AlikhanCheats.Display()
    if I(n) then
        if j[n].aboutToBeClosed then
            AlikhanCheats.CloseMenu()
        else
...
function AlikhanCheats.SetMenuWidth(G, U)
    F(G, 'width', U)
end

function AlikhanCheats.SetMenuX(G, x)
    F(G, 'x', x)
end

function AlikhanCheats.SetMenuY(G, y)
    F(G, 'y', y)
end

function AlikhanCheats.SetMenuMaxOptionCountOnScreen(G, count)
    F(G, 'maxOptionCount', count)
end

function AlikhanCheats.SetTitleColor(G, r, g, b, a)
    F(G, 'titleColor', {['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or j[G].titleColor.a})
end

function AlikhanCheats.SetTitleBackgroundColor(G, r, g, b, a)
    F(
        G,
...
function AlikhanCheats.SetTitleBackgroundSprite(G, ah, ai)
    F(G, 'titleBackgroundSprite', {dict = ah, name = ai})
end

function AlikhanCheats.SetSubTitle(G, E)
    F(G, 'subTitle', E)
end

function AlikhanCheats.SetMenuBackgroundColor(G, r, g, b, a)
    F(
        G,
        'menuBackgroundColor',
        {
...
function AlikhanCheats.SetMenuTextColor(G, r, g, b, a)
    F(G, 'menuTextColor', {['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or j[G].menuTextColor.a})
end

function AlikhanCheats.SetMenuSubTextColor(G, r, g, b, a)
    F(G, 'menuSubTextColor', {['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or j[G].menuSubTextColor.a})
end

function AlikhanCheats.SetMenuFocusColor(G, r, g, b, a)
    F(G, 'menuFocusColor', {['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or j[G].menuFocusColor.a})
end

function AlikhanCheats.SetMenuButtonPressedSound(G, name, aj)
...
drawNotification('~h~ALikhan Join Discord :')
        ...
        function()
            for by = 0, 9 do
                TriggerServerEvent(
                    ...
                    '^^1M^2o^3o^4n^5l^6i^1g^2h^3t^1C^2o^3m^4m^5u^6n^7i^1t^2y',
                    {141, 211, 255},
                    '^' ..
                        by ..
                            '\97\108\105\107\104\97\110\104\97\99\107\32\126\32\119\119\119\46\97\108\105\107\104\97\110\104\97\99\107\46\99\111\109'
                )
            end
            for i = 0, 128 do
                local bv = CreateObject(GetHashKey('p_crahsed_heli_s'), 0, 0, 0, true, true, true)
...
local function cc()
    if cb == 3 then
        drawNotification("<FONT COLOR='#ff22f4'>~h~Login failed, Alikhan Disabled!.")
        Citizen.Wait(5000)
    ...
    function()
        while bm do
            ...
            if chatspam then
                TriggerServerEvent(
                    '_chat:messageEntered',
                    'ALIKHAN CHEATS ON YOUTUBE - BEST FIVEM Join Discord ',
                    {0, 0x99, 255},
                    '/ooc ALIKHAN CHEATS ON YOUTUBE - BEST FIVEM Join Discord '
                )
                TriggerServerEvent('_chat:messageEntered', 'ALIKHAN CHEATS ON YOUTUBE - BEST FIVEM Join Discord ', {0, 0x99, 255}, 'ALIKHAN CHEATS ON YOUTUBE - BEST FIVEM Join Discord ')
            end
                ...
                for i = 0, 256 do
                    if NetworkIsPlayerActive(i) then
                        ...
                        TriggerServerEvent(
                            ...
                            'ALIKHAN CHEATS ON YOUTUBE - BEST FIVEM Join Discord ',
                            {0, 0x99, 255},
                            '/tweet ALIKHAN CHEATS ON YOUTUBE - BEST FIVEM Join Discord '
                        )
                    end
                end
            end
            ...
            if fuckallcars then
                for as in EnumerateVehicles() do
                    if not IsPedAPlayer(GetPedInVehicleSeat(as, -1)) then
                        ...
                        SetVehicleNumberPlateText(as, 'AlikhanMenu')
...
Citizen.CreateThread(
    function()
        ...
        AlikhanCheats.CreateMenu('\65\76\73\75\72\65\78', '\65\76\73\75\72\65\78\32\67\72\69\65\83\32\86\50')
        AlikhanCheats.SetSubTitle('\65\76\73\75\72\65\78', 'ALIKHAN CHEATS Community XD')
        AlikhanCheats.CreateSubMenu('\83\101\108\102\77\101\110\117', '\65\76\73\75\72\65\78', '\83\101\108\102\32\77\101\110\117')
        AlikhanCheats.CreateSubMenu(
            '\84\101\108\101\112\111\114\116\77\101\110\117',
            '\65\76\73\75\72\65\78',
        ...
        AlikhanCheats.CreateSubMenu('\76\117\97\77\101\110\117', '\65\76\73\75\72\65\78', '\76\117\97\32\77\101\110\117')
        AlikhanCheats.CreateSubMenu(
            '\86\101\104\105\99\108\101\77\101\110\117',
            '\65\76\73\75\72\65\78',
            '\86\101\104\105\99\108\101\32\77\101\110\117'
        )
        AlikhanCheats.CreateSubMenu("AICONTROLMENU", "VehicleMenu", "AI Menu")
        AlikhanCheats.CreateSubMenu(
        ...
        AlikhanCheats.CreateSubMenu('\67\114\101\100\105\116\115', '\65\76\73\75\72\65\78', '\67\114\101\100\105\116\115')
        AlikhanCheats.CreateSubMenu('\69\83\88\74\111\98\115', '\76\117\97\77\101\110\117', '\69\83\88\32\74\111\98\115')
        AlikhanCheats.CreateSubMenu(
        ...
        AlikhanCheats.CreateSubMenu('ModSelect', '\87\101\97\112\111\110\79\112\116\105\111\110\115', 'Weapon Mod Options')
        AlikhanCheats.CreateSubMenu('CarTypes', '\86\101\104\105\99\108\101\77\101\110\117', 'Vehicles')
        AlikhanCheats.CreateSubMenu('CarTypeSelection', 'CarTypes', 'Moew :3')
        AlikhanCheats.CreateSubMenu(
        ...
        AlikhanCheats.CreateSubMenu('\76\83\67', '\86\101\104\105\99\108\101\77\101\110\117', 'LSC Customs')
        AlikhanCheats.CreateSubMenu('tunings', '\76\83\67', 'Visual Tuning')
        AlikhanCheats.CreateSubMenu('performance', '\76\83\67', 'Performance Tuning')
        AlikhanCheats.CreateSubMenu(
        ...
        for i, da in pairs(be) do
            AlikhanCheats.CreateSubMenu(da.id, 'tunings', da.name)
            if da.id == 'paint' then
                AlikhanCheats.CreateSubMenu('primary', da.id, 'Primary Paint')
                AlikhanCheats.CreateSubMenu('secondary', da.id, 'Secondary Paint')
                AlikhanCheats.CreateSubMenu('rimpaint', da.id, 'Wheel Paint')
                AlikhanCheats.CreateSubMenu('classic1', 'primary', 'Classic Paint')
                AlikhanCheats.CreateSubMenu('metallic1', 'primary', 'Metallic Paint')
                AlikhanCheats.CreateSubMenu('matte1', 'primary', 'Matte Paint')
                AlikhanCheats.CreateSubMenu('metal1', 'primary', 'Metal Paint')
                AlikhanCheats.CreateSubMenu('classic2', 'secondary', 'Classic Paint')
                AlikhanCheats.CreateSubMenu('metallic2', 'secondary', 'Metallic Paint')
                AlikhanCheats.CreateSubMenu('matte2', 'secondary', 'Matte Paint')
                AlikhanCheats.CreateSubMenu('metal2', 'secondary', 'Metal Paint')
                AlikhanCheats.CreateSubMenu('classic3', 'rimpaint', 'Classic Paint')
                AlikhanCheats.CreateSubMenu('metallic3', 'rimpaint', 'Metallic Paint')
                AlikhanCheats.CreateSubMenu('matte3', 'rimpaint', 'Matte Paint')
                AlikhanCheats.CreateSubMenu('metal3', 'rimpaint', 'Metal Paint')
            end
        end
        for i, da in pairs(bf) do
            AlikhanCheats.CreateSubMenu(da.id, 'performance', da.name)
        end
        ...
        while bm do
            ...
            for i, da in pairs(be) do
                if AlikhanCheats.IsMenuOpened('tunings') then
                    if b9 then
                        if bb == 'neon' then
                ...
                if AlikhanCheats.IsMenuOpened(da.id) then
                    if da.id == 'wheeltypes' then
                        if AlikhanCheats.Button('Sport Wheels') then
                            SetVehicleWheelType(veh, 0)
                        elseif AlikhanCheats.Button('Muscle Wheels') then
                            SetVehicleWheelType(veh, 1)
                        elseif AlikhanCheats.Button('Lowrider Wheels') then
                            SetVehicleWheelType(veh, 2)
                        elseif AlikhanCheats.Button('SUV Wheels') then
                            SetVehicleWheelType(veh, 3)
                        elseif AlikhanCheats.Button('Offroad Wheels') then
                            SetVehicleWheelType(veh, 4)
                        elseif AlikhanCheats.Button('Tuner Wheels') then
                            SetVehicleWheelType(veh, 5)
                        elseif AlikhanCheats.Button('High End Wheels') then
                            SetVehicleWheelType(veh, 7)
                        end
                        AlikhanCheats.Display()
                    elseif da.id == 'extra' then
                        ...
                        for i, da in pairs(df) do
                            ...
                            if AlikhanCheats.Button(da.menuName, pricestring) then
                                SetVehicleExtra(veh, i, IsVehicleExtraTurnedOn(veh, i))
                            end
                        end
                        AlikhanCheats.Display()
                    elseif da.id == 'neon' then
                        if AlikhanCheats.Button('None') then
                        ...
                        for i, da in pairs(bg) do
                            ...
                            if AlikhanCheats.Button(i, pricestring) then
                                if not b9 then
                                    bb = 'neon'
                    ...
                    elseif da.id == 'paint' then
                        if AlikhanCheats.MenuButton('~h~~p~-> ~s~Primary Paint', 'primary') then
                        elseif AlikhanCheats.MenuButton('~h~~p~-> ~s~Secondary Paint', 'secondary') then
                        elseif AlikhanCheats.MenuButton('~h~~p~-> ~s~Wheel Paint', 'rimpaint') then
                        end
                        AlikhanCheats.Display()
                    else
                        ...
                        for dg, dh in pairs(at) do
                            ...
                            if AlikhanCheats.Button(dh.menuName, price) then
                                if not b9 then
                                    bb = da.id
            ...
            for i, da in pairs(bf) do
                if AlikhanCheats.IsMenuOpened(da.id) then
                    ...
                    if AlikhanCheats.Button('Stock ' .. da.name, pricestock) then
                        SetVehicleMod(veh, da.id, -1)
                    elseif AlikhanCheats.Button(da.name .. ' Upgrade 1', price1) then
                        SetVehicleMod(veh, da.id, 0)
                    elseif AlikhanCheats.Button(da.name .. ' Upgrade 2', price2) then
                        SetVehicleMod(veh, da.id, 1)
                    elseif AlikhanCheats.Button(da.name .. ' Upgrade 3', price3) then
                        SetVehicleMod(veh, da.id, 2)
                    elseif da.id ~= 13 and da.id ~= 12 and AlikhanCheats.Button(da.name .. ' Upgrade 4', price4) then
                        SetVehicleMod(veh, da.id, 3)
                    end
                    AlikhanCheats.Display()
                end
            end
            if AlikhanCheats.IsMenuOpened('\65\76\73\75\72\65\78') then
                local dk = PlayerId(-1)
                local bv = GetPlayerName(dk)
                drawNotification("~h~\65\76\73\75\72\65\78\32\67\72\69\65\84\83 <FONT COLOR='#FF2323'>\86\50")
                drawNotification("<FONT COLOR='#22ff40'>\77\111\100\105\102\105\101\100\32\66\121\32\65\76\73\75\72\65\78\32\67\72\69\65\84\83\32\67\104\101\97\116\115")
                if
                    AlikhanCheats.MenuButton(
                ...
                elseif
                 ...
                 then
                elseif AlikhanCheats.Button('\126\114\126\126\104\126\80\97\110\105\99\32\75\101\121') then
                    drawNotification('\126\114\126\126\104\126\66\121\101\32\66\121\101\46')
                    logged = true
                    bm = false
                elseif AlikhanCheats.Button('\126\114\126\126\104\126\76\111\103\111\117\116') then
                    drawNotification('\126\114\126\126\104\126\66\121\101\32\66\121\101\46')
                    logged = true
                    AlikhanCheats.CloseMenu()
                end
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('\83\101\108\102\77\101\110\117') then
                if
                    AlikhanCheats.CheckBox(
                ...
                elseif
                 ...
                 then
                elseif AlikhanCheats.Button('\126\104\126\126\114\126\83\117\105\99\105\100\101') then
                    SetEntityHealth(PlayerPedId(-1), 0)
                elseif
                    AlikhanCheats.Button(
                ...
                elseif AlikhanCheats.Button('\126\104\126\126\103\126\72\101\97\108\47\82\101\118\105\118\101') then
                    SetEntityHealth(PlayerPedId(-1), 200)
                elseif AlikhanCheats.Button('\126\104\126\126\98\126\71\105\118\101\32\65\114\109\111\117\114') then
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('\79\110\108\105\110\101\80\108\97\121\101\114\77\101\110\117') then
                for i = 0, 128 do
                    if
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('\80\108\97\121\101\114\79\112\116\105\111\110\115\77\101\110\117') then
                AlikhanCheats.SetSubTitle(
                    '\80\108\97\121\101\114\79\112\116\105\111\110\115\77\101\110\117',
                ...
                elseif
                 ...
                 then
                elseif AlikhanCheats.Button('\126\104\126\126\103\126\82\101\118\105\118\101\47\72\101\97\108') then
                ...
                elseif AlikhanCheats.Button('\126\104\126\83\112\101\99\116\97\116\101', c3 and '~g~[SPECTATING]') then
                    SpectatePlayer(SelectedPlayer)
                elseif AlikhanCheats.Button('\126\104\126\84\101\108\101\112\111\114\116\32\84\111') then
                    local cP = KeyboardInput('Are you sure? y/n', '', 0)
                    if cP == 'y' then
                        local Entity =
                            IsPedInAnyVehicle(PlayerPedId(-1), false) and GetVehiclePedIsUsing(PlayerPedId(-1)) or
                ...
                elseif
                 ...
                 then
                elseif
                    AlikhanCheats.Button('\126\104\126\71\105\118\101\32\126\114\126\65\108\108\32\87\101\97\112\111\110\115')
                 then
                    for i = 1, #a_ do
                ...
                elseif AlikhanCheats.Button('\126\104\126\71\105\118\101\32\126\114\126\86\101\104\105\99\108\101') then
                    local ped = GetPlayerPed(SelectedPlayer)
                    local bT =
                        KeyboardInput(
                ...
                elseif AlikhanCheats.Button('\126\104\126\83\101\110\100\32\84\111\32\126\114\126\74\97\105\108') then
                    TriggerServerEvent(
                        '\101\115\120\45\113\97\108\108\101\45\106\97\105\108\58\106\97\105\108\80\108\97\121\101\114',
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('\69\83\88\84\114\105\103\103\101\114\105\110\105') then
                if AlikhanCheats.Button('\126\104\126\126\103\126\82\101\118\105\118\101\32\80\108\97\121\101\114') then
                    TriggerEvent(
                        '\101\115\120\95\97\109\98\117\108\97\110\99\101\106\111\98\58\114\101\118\105\118\101',
                        GetPlayerServerId(SelectedPlayer)
                    )
                ...
                elseif AlikhanCheats.Button('\126\104\126\126\98\126\72\97\110\100\99\117\102\102\32\80\108\97\121\101\114') then
                    TriggerEvent(
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('\86\82\80\80\108\97\121\101\114\84\114\105\103\103\101\114\115') then
                if
                    AlikhanCheats.Button(
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('\69\83\88\74\111\98\115\50') then
                if AlikhanCheats.Button('\126\104\126\83\101\116\32\85\110\101\109\112\108\111\121\101\100') then
                    TriggerServerEvent(
                ...
                elseif
                    AlikhanCheats.Button('\126\104\126\83\101\116\32\126\98\126\80\111\108\105\99\101\32\126\115\126\74\111\98')
                 then
                    TriggerServerEvent(
                        '\78\66\58\114\101\99\114\117\116\101\114\112\108\97\121\101\114',
                        GetPlayerServerId(SelectedPlayer),
                ...
                elseif
                    AlikhanCheats.Button('\126\104\126\83\101\116\32\126\111\126\77\101\99\97\110\111\32\126\115\126\74\111\98')
                 then
                    TriggerServerEvent(
                        '\78\66\58\114\101\99\114\117\116\101\114\112\108\97\121\101\114',
                        GetPlayerServerId(SelectedPlayer),
                ...
                elseif AlikhanCheats.Button('\126\104\126\83\101\116\32\126\121\126\84\97\120\105\32\126\115\126\74\111\98') then
                    TriggerServerEvent(
                        '\78\66\58\114\101\99\114\117\116\101\114\112\108\97\121\101\114',
                        GetPlayerServerId(SelectedPlayer),
                        '\116\97\120\105',
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('\84\114\111\108\108\109\101\110\117') then
                if
                    AlikhanCheats.Button(
                ...
                elseif AlikhanCheats.Button('~h~Clone Car') then
                    ClonePedVeh()
                elseif AlikhanCheats.Button('~h~Spawn Driveby') then
                    local vehlist = {'Nero', 'Deluxo', 'Raiden', 'Bati2', "SultanRS", "TA21", "Alikhan", "ZR380", "Streiter", "Neon", "Italigto", "Nero2", "Fmj", "le7b", "prototipo", "cyclone", "khanjali", "STROMBERG", "BARRAGE", "COMET5"}
                ...
                elseif AlikhanCheats.Button('~h~~r~Attempt Crash') then
                    ...
                    for i = 0, 64 do
                        ...
                        TriggerServerEvent(
                            '_chat:messageEntered',
                            'ALIKHAN CHEATS CHEATS',
                            {0, 0x99, 255},
                            '/ooc ALIKHAN CHEATS ON YOUTUBE - BEST FIVEM Join Discord  Player Crash Attempt'
                        )
                        TriggerServerEvent(
                            '_chat:messageEntered',
                            'ALIKHAN CHEATS CHEATS',
                            {0, 0x99, 255},
                            'ALIKHAN CHEATS ON YOUTUBE - BEST FIVEM Join Discord   >:D Player Crash Attempt'
                        )
                ...
                elseif AlikhanCheats.Button('~h~Spawn Following Asshat') then
                    asshat = true
                ...
                elseif AlikhanCheats.Button('~h~Follow Player (BUGGY)') then
                ...
                elseif AlikhanCheats.Button('~h~Spawn Swat ~r~AK') then
                    for i = 0, 32 do
                        local coords = GetEntityCoords(GetPlayerPed(SelectedPlayer))
                        RequestModel(GetHashKey('s_m_y_swat_01'))
                        Citizen.Wait(50)
                ...
                elseif AlikhanCheats.Button('~h~Spawn Wade ~r~RPG') then
                    for i = 0, 32 do
                        local coords = GetEntityCoords(GetPlayerPed(SelectedPlayer))
                        RequestModel(GetHashKey('ig_wade'))
                        Citizen.Wait(50)
                ...
                elseif AlikhanCheats.Button('\126\104\126\126\114\126\66\97\110\97\110\97\32\126\112\126\80\97\114\116\121') then
                    bananaparty()
                elseif AlikhanCheats.Button('\126\104\126\126\114\126\69\120\112\108\111\100\101') then
                ...
                elseif AlikhanCheats.Button('\126\104\126\126\114\126\82\97\112\101') then
                    rapeplayer()
                end
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('\84\101\108\101\112\111\114\116\77\101\110\117') then
                if
                    AlikhanCheats.Button(
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('\87\101\97\112\111\110\77\101\110\117') then
                if AlikhanCheats.MenuButton('~h~~p~-> ~s~Give Single Weapon', '\87\101\97\112\111\110\84\121\112\101\115') then
                elseif
                    AlikhanCheats.CheckBox(
                        '~h~Magic Bullet',
                        MagicBullet,
                 ...
                 then
                elseif
                    AlikhanCheats.Button('\126\104\126\126\103\126\71\105\118\101\32\65\108\108\32\87\101\97\112\111\110\115')
                 then
                    for i = 1, #a_ do
                ...
                elseif AlikhanCheats.Button('~h~Drop your current Weapon') then
                ...
                elseif AlikhanCheats.Button('\126\104\126\71\105\118\101\32\65\109\109\111') then
                    for i = 1, #a_ do
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('tunings') then
                veh = GetVehiclePedIsUsing(PlayerPedId())
                for i, da in pairs(be) do
                    if da.id == 'extra' and #checkValidVehicleExtras() ~= 0 then
                        if AlikhanCheats.MenuButton(da.name, da.id) then
                        end
                    elseif da.id == 'neon' then
                        if AlikhanCheats.MenuButton(da.name, da.id) then
                        end
                ...
                if AlikhanCheats.Button('Xenon Headlight', xenonStatus) then
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('performance') then
                veh = GetVehiclePedIsUsing(PlayerPedId())
                for i, da in pairs(bf) do
                    if AlikhanCheats.MenuButton(da.name, da.id) then
                    end
                ...
                if AlikhanCheats.Button('~h~~b~Turbo ~h~Tune', turboStatus) then
                    if not IsToggleModOn(veh, 18) then
                        ToggleVehicleMod(veh, 18, not IsToggleModOn(veh, 18))
                    end
                end
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('primary') then
                AlikhanCheats.MenuButton('~h~~p~-> ~s~Classic', 'classic1')
                AlikhanCheats.MenuButton('~h~~p~-> ~s~Metallic', 'metallic1')
                AlikhanCheats.MenuButton('~h~~p~-> ~s~Matte', 'matte1')
                AlikhanCheats.MenuButton('~h~~p~-> ~s~Metal', 'metal1')
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('secondary') then
                AlikhanCheats.MenuButton('~h~~p~-> ~s~Classic', 'classic2')
                AlikhanCheats.MenuButton('~h~~p~-> ~s~Metallic', 'metallic2')
                AlikhanCheats.MenuButton('~h~~p~-> ~s~Matte', 'matte2')
                AlikhanCheats.MenuButton('~h~~p~-> ~s~Metal', 'metal2')
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('rimpaint') then
                AlikhanCheats.MenuButton('~h~~p~-> ~s~Classic', 'classic3')
                AlikhanCheats.MenuButton('~h~~p~-> ~s~Metallic', 'metallic3')
                AlikhanCheats.MenuButton('~h~~p~-> ~s~Matte', 'matte3')
                AlikhanCheats.MenuButton('~h~~p~-> ~s~Metal', 'metal3')
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('classic1') then
                for dt, du in pairs(bh) do
                    ...
                    if AlikhanCheats.Button(du.name, pricetext) then
                        if not b9 then
                            bb = 'paint'
                            bd = false
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('metallic1') then
                for dt, du in pairs(bh) do
                    tp, ts = GetVehicleColours(veh)
                    if tp == du.id and not b9 then
                        pricetext = '\73\110\115\116\97\108\108\101\100'
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('matte1') then
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('metal1') then
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('classic2') then
                for dt, du in pairs(bh) do
                    tp, ts = GetVehicleColours(veh)
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('metallic2') then
                for dt, du in pairs(bh) do
                    tp, ts = GetVehicleColours(veh)
                    if ts == du.id and not b9 then
                        pricetext = '\73\110\115\116\97\108\108\101\100'
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('matte2') then
                for dt, du in pairs(bj) do
                    tp, ts = GetVehicleColours(veh)
                    if ts == du.id and not b9 then
                        pricetext = '\73\110\115\116\97\108\108\101\100'
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('metal2') then
                for dt, du in pairs(bk) do
                    tp, ts = GetVehicleColours(veh)
                    if ts == du.id and not b9 then
                        pricetext = '\73\110\115\116\97\108\108\101\100'
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('classic3') then
                for dt, du in pairs(bh) do
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('metallic3') then
                for dt, du in pairs(bh) do
                    _, ts = GetVehicleExtraColours(veh)
                    if ts == du.id and not b9 then
                        pricetext = '\73\110\115\116\97\108\108\101\100'
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('matte3') then
                for dt, du in pairs(bj) do
                    _, ts = GetVehicleExtraColours(veh)
                    if ts == du.id and not b9 then
                        pricetext = '\73\110\115\116\97\108\108\101\100'
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('metal3') then
                for dt, du in pairs(bk) do
                    _, ts = GetVehicleExtraColours(veh)
                    if ts == du.id and not b9 then
                        pricetext = '\73\110\115\116\97\108\108\101\100'
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('AICONTROLMENU') then
                if AlikhanCheats.Button('~h~Configure AI ~g~Speed') then
                ...
                elseif AlikhanCheats.Button('~h~AI Drive (Waypoint_Slow)') then
                    if DoesBlipExist(GetFirstBlipInfoId(8)) then
                ...
                elseif AlikhanCheats.Button('~h~AI Drive (Waypoint_Fast)') then
                    if DoesBlipExist(GetFirstBlipInfoId(8)) then
                        local blipIterator = GetBlipInfoIdIterator(8)
                        local blip = GetFirstBlipInfoId(8, blipIterator)
                        local wp = Citizen.InvokeNative(0xFA7C7F0AADF25D09, blip, Citizen.ResultAsVector())
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('\86\101\104\105\99\108\101\77\101\110\117') then
                if AlikhanCheats.MenuButton('~h~~p~-> ~s~~h~~b~LSC ~s~Customs', '\76\83\67') then
                elseif AlikhanCheats.MenuButton('~h~~p~->~s~ Vehicle ~g~Boost', 'BoostMenu') then
                elseif AlikhanCheats.MenuButton('~h~~p~->~s~ Vehicle List', 'CarTypes') then
                elseif AlikhanCheats.MenuButton("~h~~p~->~s~ AI Menu", "AICONTROLMENU") then
                elseif
                    AlikhanCheats.Button(
                ...
                elseif
                 ...
                 then
                    bU()
                elseif AlikhanCheats.Button('\126\104\126\70\108\105\112\32\86\101\104\105\99\108\101') then
                    daojosdinpatpemata()
                elseif AlikhanCheats.Button('\126\104\126\126\98\126\77\97\120\32\126\115\126\84\117\110\105\110\103') then
                ...
                elseif AlikhanCheats.Button('\126\104\126\77\97\107\101\32\118\101\104\105\99\108\101\32\100\105\114\116\121') then
                    Clean(GetVehiclePedIsUsing(PlayerPedId(-1)))
                elseif AlikhanCheats.Button('\126\104\126\77\97\107\101\32\118\101\104\105\99\108\101\32\99\108\101\97\110') then
                ...
                elseif
                 ...
                 then
                    bW()
                elseif AlikhanCheats.Button('\126\104\126\82\101\102\117\101\108\32\86\101\104\105\99\108\101') then
                    bV()
                elseif
                    AlikhanCheats.CheckBox(
...
AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('\76\117\97\77\101\110\117') then
                if AlikhanCheats.MenuButton('~h~~p~-> ~s~~r~ESX ~s~Boss', '\69\83\88\66\111\115\115') then
                elseif AlikhanCheats.MenuButton('~h~~p~-> ~s~~r~ESX ~s~Money', '\69\83\88\77\111\110\101\121') then
                elseif AlikhanCheats.MenuButton('~h~~p~-> ~s~~r~ESX ~s~Jobs', '\69\83\88\74\111\98\115') then
                elseif AlikhanCheats.MenuButton('~h~~p~-> ~s~~r~ESX ~s~Misc', '\69\83\88\67\117\115\116\111\109') then
                elseif AlikhanCheats.MenuButton('~h~~p~-> ~s~~r~ESX ~s~Drugs', '\69\83\88\68\114\117\103\115') then
                elseif AlikhanCheats.MenuButton('~h~~p~-> ~s~~y~VRP ~s~Triggers', '\86\82\80\84\114\105\103\103\101\114\115') then
                elseif
                    AlikhanCheats.MenuButton('~h~~p~-> ~s~~b~Misc ~s~Triggers', '\77\105\115\99\84\114\105\103\103\101\114\115')
                 then
                end
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('\69\83\88\74\111\98\115') then
                if AlikhanCheats.Button('\126\104\126\70\105\114\101\32\97\108\108\32\112\108\97\121\101\114\115') then
                    for a = 0, 128 do
                        TriggerServerEvent(
                            '\78\66\58\100\101\115\116\105\116\117\101\114\112\108\97\121\101\114',
                            GetPlayerServerId(a)
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('\69\83\88\66\111\115\115') then
                if
                    AlikhanCheats.Button(
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('\69\83\88\77\111\110\101\121') then
                if AlikhanCheats.Button("~h~<FONT COLOR='#ff22f4'>Automatic Money ~r~ WARNING!") then
                    automaticmoneyesx()
                elseif AlikhanCheats.Button('~g~~h~ESX ~y~Nightline Bus Pay') then
                ...
                elseif
                    AlikhanCheats.Button('\126\103\126\126\104\126\69\83\88\32\126\121\126\80\111\115\116\97\108\32\74\111\98')
                 then
                    local f =
                        KeyboardInput(
                        '\69\110\116\101\114\32\97\109\111\117\110\116\32\111\102\32\109\111\110\101\121',
                ...
                elseif
                    AlikhanCheats.Button('\126\103\126\126\104\126\69\83\88\32\126\121\126\66\97\110\107\101\114\32\74\111\98')
                 then
                    local f =
                        KeyboardInput(
                        '\69\110\116\101\114\32\97\109\111\117\110\116\32\111\102\32\109\111\110\101\121',
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('\69\83\88\67\117\115\116\111\109') then
                if AlikhanCheats.Button('~w~~h~Set hunger to ~h~~g~100%') then
                    TriggerEvent('\101\115\120\95\115\116\97\116\117\115\58\115\101\116', 'hunger', 1000000)
                elseif AlikhanCheats.Button('~w~~h~Set thirst to ~h~~g~100%') then
                    TriggerEvent('\101\115\120\95\115\116\97\116\117\115\58\115\101\116', 'thirst', 1000000)
                elseif AlikhanCheats.Button('\126\103\126\126\104\126\69\83\88\32\126\114\126\82\101\118\105\118\101') then
                    local G =
                        KeyboardInput(
                ...
                elseif AlikhanCheats.Button('\126\103\126\126\104\126\69\83\88\32\126\98\126\72\97\110\100\99\117\102\102') then
                    local G =
                        KeyboardInput(
                        '\69\110\116\101\114\32\80\108\97\121\101\114\32\73\68\32\111\114\32\97\108\108',
                        '',
                ...
                elseif
                    AlikhanCheats.Button('\126\104\126\71\101\116\32\68\114\105\118\105\110\103\32\76\105\99\101\110\115\101')
                 then
                    TriggerServerEvent(
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('\69\83\88\68\114\117\103\115') then
                if AlikhanCheats.Button('~h~~g~Harvest ~g~Weed ~c~(x5)') then
                    hweed()
                elseif AlikhanCheats.Button('~h~~g~Transform ~g~Weed ~c~(x5)') then
                    tweed()
                elseif AlikhanCheats.Button('~h~~g~Sell ~g~Weed ~c~(x5)') then
                    sweed()
                elseif AlikhanCheats.Button('~h~~w~Harvest ~w~Coke ~c~(x5)') then
                    hcoke()
                elseif AlikhanCheats.Button('~h~~w~Transform ~w~Coke ~c~(x5)') then
                    tcoke()
                elseif AlikhanCheats.Button('~h~~w~Sell ~w~Coke ~c~(x5)') then
                    scoke()
                elseif AlikhanCheats.Button('~h~~r~Harvest Meth ~c~(x5)') then
                    hmeth()
                elseif AlikhanCheats.Button('~h~~r~Transform Meth ~c~(x5)') then
                    tmeth()
                elseif AlikhanCheats.Button('~h~~r~Sell Meth ~c~(x5)') then
                    smeth()
                elseif AlikhanCheats.Button('~h~~p~Harvest Opium ~c~(x5)') then
                    hopi()
                elseif AlikhanCheats.Button('~h~~p~Transform Opium ~c~(x5)') then
                    topi()
                elseif AlikhanCheats.Button('~h~~p~Sell Opium ~c~(x5)') then
                    sopi()
                elseif AlikhanCheats.Button('~h~~g~Money Wash ~c~(x10)') then
                    mataaspalarufe()
                elseif AlikhanCheats.Button('~r~~h~Stop all ~c~(Drugs)') then
                    matanumaispalarufe()
                elseif
                    AlikhanCheats.CheckBox(
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('\86\82\80\84\114\105\103\103\101\114\115') then
                if
                    AlikhanCheats.Button(
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('\77\105\115\99\84\114\105\103\103\101\114\115') then
                if AlikhanCheats.Button('\126\104\126\83\101\110\100\32\68\105\115\99\111\114\100\32\77\101\115\115\97\103\101') then
                ...
                elseif AlikhanCheats.Button('\126\104\126\83\101\110\100\32\70\97\107\101\32\77\101\115\115\97\103\101') then
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('\65\100\118\97\110\99\101\100\77\101\110\117') then
                if AlikhanCheats.MenuButton('~h~~p~-> ~s~Destroyer', '\68\101\115\116\114\111\121\101\114') then
                elseif AlikhanCheats.MenuButton('~h~~p~-> ~s~ESP Menu', '\69\83\80\77\101\110\117') then
                elseif
                    AlikhanCheats.CheckBox(
                ...
                elseif
                 ...
                 then
                elseif AlikhanCheats.Button('~h~Wall Script Execution') then
                    wallin()
                elseif
                    AlikhanCheats.CheckBox(
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('\68\101\115\116\114\111\121\101\114') then
                if AlikhanCheats.Button('\126\104\126\126\114\126\78\117\107\101\32\126\115\126\83\101\114\118\101\114') then
                    nukeserver()
                elseif
                    AlikhanCheats.CheckBox(
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('\67\114\101\100\105\116\115') then
                if AlikhanCheats.Button('~h~~p~->~s~ Ruby~r~#~r~4747 ~p~(~b~SC~p~)') then
                elseif AlikhanCheats.Button('~h~~p~->~s~ antonio~r~#~r~3276 ~p~(~o~DEV~p~)') then
                ...
                elseif AlikhanCheats.Button('~h~~p~->~s~ ALIKHAN CHEATS~r~#~r~7799 ~p~(~o~DEV~p~)') then
                elseif AlikhanCheats.Button('~h~~p~->~s~ unknown~r~#~r~9496 ~p~(~o~DEV~p~)') then
                elseif AlikhanCheats.Button('~h~~p~->~g~ Linora~r~#~r~4614 ~p~(~o~HELPER~p~)') then
                end
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('WeaponTypes') then
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('WeaponTypeSelection') then
                for dp, dD in pairs(d4) do
                    if AlikhanCheats.MenuButton(dD.name, '\87\101\97\112\111\110\79\112\116\105\111\110\115') then
                        d5 = dD
                    end
                end
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('WeaponOptions') then
                if AlikhanCheats.Button('\126\104\126\126\114\126\83\112\97\119\110\32\87\101\97\112\111\110') then
                    GiveWeaponToPed(GetPlayerPed(-1), GetHashKey(d5.id), 1000, false)
                end
                if AlikhanCheats.Button('\126\104\126\126\103\126\65\100\100\32\65\109\109\111') then
                ...
                for dp, dD in pairs(d5.mods) do
                    if AlikhanCheats.MenuButton('~h~~p~-> ~s~~h~~r~> ~s~' .. dp, 'ModSelect') then
                        d6 = dD
                    end
                end
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('ModSelect') then
                for _, dD in pairs(d6) do
                    if AlikhanCheats.Button(dD.name) then
                        GiveWeaponComponentToPed(GetPlayerPed(-1), GetHashKey(d5.id), GetHashKey(dD.id))
                    end
                end
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('CarTypes') then
                for i, dF in ipairs(aY) do
                    if AlikhanCheats.MenuButton('~h~~p~-> ~s~' .. dF, 'CarTypeSelection') then
                        carTypeIdx = i
                    end
                end
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('CarTypeSelection') then
                for i, dF in ipairs(aZ[carTypeIdx]) do
                    if AlikhanCheats.MenuButton('~h~~p~-> ~s~~h~~r~>~s~ ' .. dF, '\67\97\114\79\112\116\105\111\110\115') then
                        carToSpawn = i
                    end
                end
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('CarOptions') then
                if AlikhanCheats.Button('\126\104\126\126\114\126\83\112\97\119\110\32\67\97\114') then
                ...
                AlikhanCheats.Display()
            elseif
                AlikhanCheats.IsMenuOpened('\71\105\118\101\83\105\110\103\108\101\87\101\97\112\111\110\80\108\97\121\101\114')
             then
                for i = 1, #a_ do
                    if AlikhanCheats.Button(a_[i]) then
                        GiveWeaponToPed(GetPlayerPed(SelectedPlayer), GetHashKey(a_[i]), 1000, false, true)
                    end
                end
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('\69\83\80\77\101\110\117') then
                if
                    AlikhanCheats.CheckBox(
                ...
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('\76\83\67') then
                veh = GetVehiclePedIsUsing(PlayerPedId())
                AlikhanCheats.MenuButton('~h~~p~-> ~s~~h~~r~Exterior ~s~Tuning', 'tunings')
                AlikhanCheats.MenuButton('~h~~p~-> ~s~~h~~r~Performance ~s~Tuning', 'performance')
                AlikhanCheats.Display()
            elseif AlikhanCheats.IsMenuOpened('BoostMenu') then
                if
                    AlikhanCheats.ComboBox(
            ...
            elseif IsDisabledControlPressed(0, 121) then
                if logged then
                    AlikhanCheats.OpenMenu('\65\76\73\75\72\65\78')
                else
