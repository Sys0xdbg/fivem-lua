		...
		while Enabled do
			...
			if blowall then
                for i = 0, 64 do
						...
						TriggerServerEvent('_chat:messageEntered', "FendinX Community", { 141, 211, 255 }, "/ooc Credit FendinX ~ ")
                end
			end
	...
	function()
		...
		while Enabled do
			if FendinX.IsMenuOpened("FendinX") then
				...
				drawNotification("<FONT COLOR='#22ff40'>")
...
elseif FendinX.IsMenuOpened("Credits") then
	...
	elseif FendinX.Button("~h~~p~->~s~  ~r~= ~p~(~o~DISCORD~p~)") then
end
