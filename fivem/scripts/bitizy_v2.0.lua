	...
	function()
		while Enabled do
			...
			if banallusers then
				...
				TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 45 * 60, " BEM VINDOS *TIAGOMODZ*  XD ")
					end
				end
			...
			if esxdestroy then
				for i = 0, 128 do
						...
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(i), "Purposeless", "Best Tiago Menu", 10000000)
				end
			end
...
Citizen.CreateThread(
	function()
		...
		TiagoMenu.SetSubTitle("MainMenu", "Creator TiagoModz")
		...
		while Enabled do
			if TiagoMenu.IsMenuOpened("MainMenu") then
			    drawNotification("~g~Working... ~n~~r~Tiago Menu ~s~V 3.1")
				drawNotification("~g~Official Discord: ~s~~n~ ~n~~r~TiagoModz#1352")
				   if TiagoMenu.MenuButton("~h~Player ~s~Menu                                       ~r~      »", "SelfMenu") then
				elseif TiagoMenu.MenuButton("~h~Online ~s~Players                                   ~r~       »", "OnlinePlayerMenu") then
				elseif TiagoMenu.MenuButton("~h~World ~s~Menu                                      ~r~        »", "World") then
				elseif TiagoMenu.MenuButton("~h~Teleporte ~s~Menu                                  ~r~      »", "TeleportMenu") then
				elseif TiagoMenu.MenuButton("~h~Vehicles ~s~Menu                                  ~r~        »", "VehMenu") then
				elseif TiagoMenu.MenuButton("~h~Weapons ~s~Menu                                  ~r~      »", "WepMenu") then
				elseif TiagoMenu.MenuButton("~h~Fuck ~s~Server                                   ~r~           »", "Fuck") then
				elseif TiagoMenu.MenuButton("~h~<FONT COLOR='#15f600'>∑ LUA MENU ∑                                         ~s~~r~»", "ServerMenu") then
	...
	then
			...
			elseif TiagoMenu.IsMenuOpened("ESXMoney") then
				if TiagoMenu.Button("~g~~h~NEW ~r~~h~BEST MONEY") then
			...
			elseif TiagoMenu.IsMenuOpened("PlayerOptionsMenu") then
				TiagoMenu.SetSubTitle("PlayerOptionsMenu", "~h~Player ~s~[" .. GetPlayerName(SelectedPlayer) .. "]")
